...
def wpp_ia_conectar():
    ...
    if err: return err
    d = request.json or {}
    evolution_url = (d.get('evolution_url') or '').rstrip('/')
    api_key       = d.get('api_key', '')
    instance_name = 'musa_' + str(sid)
    if not evolution_url:
        return jsonify({'ok': False, 'erro': 'URL da Evolution API é obrigatória'})
...
def wpp_ia_status():
    ...
    evolution_url = conn['evolution_url']
    api_key       = conn['instance_key']
