...
def _gerar_msg_lembrete_semanal(sid, svc, primeiro_nome, nome_salao, cfg_ia):
    ...
    if modo == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            import requests as _rq
            prompt = ('Escreva uma mensagem curta e simpática de WhatsApp convidando a cliente ' + primeiro_nome +
                      ' a agendar o serviço "' + svc['nome'] + '" esta semana no salão ' + nome_salao + '. '
                      'Tom leve, natural, sem soar repetitiva. Máximo 3 linhas, 1 emoji. Sem aspas.')
            r = _rq.post('https://api.groq.com/openai/v1/chat/completions',
                headers={'Authorization': 'Bearer ' + cfg_ia['groq_key'], 'Content-Type': 'application/json'},
                json={'model': 'llama-3.1-8b-instant', 'messages': [{'role':'user','content':prompt}],
                      'max_tokens': 200, 'temperature': 0.85}, timeout=15)
            if r.status_code == 200:
    ...
    return ('Oi ' + primeiro_nome + '! 💅 Tudo bem?\n\n'
            'Passando para saber se você gostaria de agendar seu ' + svc['nome'] +
            ' esta semana aqui no ' + nome_salao + '.\n\n'
            'Responda aqui ou mande uma mensagem para reservar seu horário! 💕')
...
def lembrete_semanal_disparar():
    ...
    total = _run_lembrete_semanal(sids, hoje, dia_semana, True)
    return jsonify({'ok': True, 'enviados': total})


def _run_lembrete_semanal(sids, hoje, dia_semana, forcar):
