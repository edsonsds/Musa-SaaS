...
@app.route('/entrar')
def entrar():
    # Só limpar sessão se explicitamente pedido com ?sair=1
    if request.args.get('sair') == '1':
        session.clear()
    elif session.get('salon_id'):
        return redirect('/?salon_id=' + str(session['salon_id']))
...
def index():
    ...
    # Não limpar sessão aqui — verificar se já tem sessão ativa
    if session.get('salon_id'):
        # Redirecionar para o sistema do salão logado
        return redirect('/?salon_id=' + str(session['salon_id']))
    return send_from_directory('static', 'entrar.html')
...
def wpp_ia_conectar():
    ...
    db_commit()
    return jsonify({'ok': True, 'qr': qr, 'instance': instance_name})
...
def wpp_ia_testar():
    ...
    import os as _os
    _os.environ['GROQ_API_KEY'] = cfg['groq_key']
...
def wpp_webhook(sid):
    ...
    def enviar_msg(numero, texto_resp):
        ...
        try:
            ...
            req = _ur.Request(
                conn['evolution_url'] + '/message/sendText/' + conn['instance_name'],
                data=payload,
                headers={
                    'Content-Type': 'application/json',
                    'apikey': conn['instance_key']
                },
                method='POST'
            )
            _ur.urlopen(req, timeout=10)
        except Exception as ex:
            print('Erro ao enviar WPP:', ex)
    ...
    import os as _os2
    _os2.environ['GROQ_API_KEY'] = cfg.get('groq_key', '')
