...
def init_db():
    ...
    # Lembrete semanal de manicure/serviços específicos
    cur.execute("""
    CREATE TABLE IF NOT EXISTS lembrete_semanal_log (
        ...
        UNIQUE(salon_id, cli_id, enviado_em)
    )""")
    try:
        cur.execute("""CREATE UNIQUE INDEX IF NOT EXISTS idx_lemb_sem_semana
            ON lembrete_semanal_log(salon_id, cli_id,
            date_trunc('week', enviado_em))""")
    except Exception: pass

    # Clientes bloqueadas de receber lembrete semanal
    cur.execute("""
    ...
    # Migrations: cuidado pós-atendimento
    try:
        ...
        pass
    # Log de envio de pós-atendimento (não reenviar para o mesmo atendimento)
    cur.execute("""
...
@app.route('/api/servicos/diagnostico', methods=['GET'])
def servicos_diag():
    """Verifica quantos serviços existem no banco (ativos e inativos)."""
    sid, err = require_salon()
    if err: return err
    total = db_exec("SELECT COUNT(*) as n FROM servicos WHERE salon_id=%s", (sid,), 'one')
    ativos = db_exec("SELECT COUNT(*) as n FROM servicos WHERE salon_id=%s AND ativo=1", (sid,), 'one')
    inativos = db_exec("SELECT COUNT(*) as n FROM servicos WHERE salon_id=%s AND (ativo=0 OR ativo IS NULL)", (sid,), 'one')
    amostra = db_exec("SELECT id,nome,categoria,preco,ativo FROM servicos WHERE salon_id=%s ORDER BY id DESC LIMIT 10", (sid,), 'all')
    return jsonify({
        'salon_id': sid,
        'total': total['n'] if total else 0,
        'ativos': ativos['n'] if ativos else 0,
        'inativos': inativos['n'] if inativos else 0,
        'amostra': [dict(r) for r in (amostra or [])]
    })
...
def servico_lembrete_semanal(svid):
    """Ativa/desativa o lembrete semanal para um serviço, e salva modo (fixo/ia) + texto."""
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
...
def servico_lembrete_semanal_sugestao(svid):
    """Gera com IA um texto sugerido para o lembrete semanal deste serviço."""
    sid, err = require_salon()
    if err: return err
    ...
    try:
        ...
        prompt = ('Escreva uma mensagem curta e simpática de WhatsApp convidando uma cliente de salão de beleza '
                  ...
                  'Tom leve, sem parecer repetitiva ou insistente — ela pode já ter recebido mensagens parecidas antes. '
                  'Máximo 3 linhas, 1 emoji. Responda só a mensagem, sem aspas.')
...
def lembrete_semanal_lista():
    """Lista de clientes que vão receber o lembrete semanal nesta semana.
    Divide: metade na terça (dia par do id), metade na quarta (dia ímpar do id).
    Exclui: quem já recebeu esta semana, quem está bloqueado, quem tem agendamento na semana."""
    sid, err = require_salon()
    if err: return err
    import datetime as _dt
    hoje = today_br()
    ...
    # Clientes que fizeram serviços com lembrete_semanal=1 nos últimos 60 dias
    clientes = db_exec("""
    ...
    # Dividir: id par → terça, id ímpar → quarta
...
def lembrete_semanal_bloquear():
    """Bloqueia permanentemente uma cliente de receber o lembrete semanal."""
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    cli_id = d.get('cli_id')
...
def lembrete_semanal_desbloquear():
    """Remove o bloqueio de uma cliente."""
    sid, err = require_salon()
    if err: return err
...
def lembrete_semanal_disparar():
    """Dispara o lembrete semanal. Chamado pelo cron às terças e quartas.
    Terça: clientes com id par. Quarta: clientes com id ímpar."""
    import datetime as _dt
    ...
    hoje = today_br()
    dia_semana = hoje.isoweekday()  # 2=terça, 3=quarta
    if not forcar and dia_semana not in (2, 3):
        return jsonify({'ok': True, 'msg': 'Não é terça nem quarta, nada enviado.'})

    total = 0
    for s in sids:
        # Serviços com lembrete ativo neste salão (cada um com seu próprio texto/modo)
        svcs_lem = db_exec(
        ...
        # Processa cada serviço separadamente (textos diferentes por serviço)
        ja_notificado_nesta_rodada = set()  # evita duplicar se cliente fez 2 serviços com lembrete
        for svc in (svcs_lem or []):
            ...
            # Filtrar pelo dia: terça=par, quarta=ímpar (ou tudo se forçar)
            if not forcar:
...
def _gerar_msg_lembrete_semanal(sid, svc, primeiro_nome, nome_salao, cfg_ia):
    """Gera a mensagem do lembrete semanal para um serviço: IA, texto fixo, ou padrão."""
...
def retorno_alertas_previa():
    """Lista prévia de todos os alertas de retorno (vencidos, próximos e futuros).
    Mostra o status de cada um para planejamento."""
    sid, err = require_salon()
    if err: return err
...
def servico_pos(svid):
    """Salva o cuidado pós-atendimento de um serviço (ativo + texto)."""
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
...
def servico_pos_sugestao(svid):
    """Gera com IA um texto sugerido de cuidados pós-procedimento para o serviço."""
    sid, err = require_salon()
    if err: return err
    svc = db_exec("SELECT nome FROM servicos WHERE id=%s AND salon_id=%s", (svid, sid), 'one')
    if not svc:
    ...
    cfg = db_exec("SELECT groq_key FROM wpp_ia_config WHERE salon_id=%s", (sid,), 'one')
    nome_svc = svc['nome']
    # Sem chave Groq, devolve um texto padrão amigável
    if not cfg or not cfg.get('groq_key'):
        ...
        return jsonify({'ok': True, 'texto': txt, 'fonte': 'padrao'})
    # Com chave, pede à IA um texto curto
    try:
        import requests as _rq
...
def pos_atendimento_processar():
    """Envia mensagens de pós-atendimento ~2h após a conclusão.
    Chamado pelo cron (?key=) ou manualmente. Só envia para serviços com pos_ativo=1."""
    key = request.args.get('key', '')
    keyrow = db_exec("SELECT valor FROM sistema_global WHERE chave='cron_key'", fetch='one')
    cron_key = keyrow['valor'] if keyrow else 'musa_cron_2024'
    if not (key and key == cron_key):
        ...
        if err: return err
    # Busca atendimentos concluídos cujo serviço tem pós ativo, concluídos entre 2h e ~4h atrás,
    # e que ainda não receberam a mensagem.
    enviados = 0
    pendentes = db_exec("""
    ...
    for p in (pendentes or []):
        ...
        try:
            ...
            ok = False
        # Registra mesmo se falhou, para não ficar tentando infinitamente
...
def agendamentos():
    ...
    if request.method == 'GET':
        ...
        return jsonify([dict(r) for r in rows])
    d = request.json
    # VERIFICAR BLOQUEIO: impedir agendamento em horário bloqueado do profissional
    try:
        novo_data = d['data']  # 'YYYY-MM-DD'
        novo_ini = d['h_ini']  # 'HH:MM'
        novo_fim = d['h_fim']
        pro = d['pro_id']
        # data_inicio/data_fim guardam "YYYY-MM-DD HH:MM"
...
# ─── COMISSÕES ────────────────────────────────────────────────────────────────
@app.route('/api/diagnostico-lembrete-cliente', methods=['GET'])
def diagnostico_lembrete_cliente():
    """Investiga por que um lembrete foi enviado a um cliente específico (por nome).
    Mostra TODOS os agendamentos dele, status, e os logs de lembrete já enviados."""
    sid, err = require_salon()
    if err: return err
    nome = request.args.get('nome', '').strip()
    if not nome:
        return jsonify({'erro': 'Informe ?nome=NomeDoCliente'})
    clientes = db_exec("SELECT id, nome, tel, ativo FROM clientes WHERE salon_id=%s AND nome ILIKE %s",
                        (sid, '%' + nome + '%'), 'all')
    out = []
    for c in (clientes or []):
        c = dict(c)
        ags = db_exec("""SELECT id, data, h_ini, h_fim, status, criado_em, svc_id
            FROM agendamentos WHERE salon_id=%s AND cli_id=%s
            ORDER BY data DESC, h_ini DESC LIMIT 20""", (sid, c['id']), 'all')
        logs = db_exec("""SELECT ll.id, ll.ag_id, ll.antecedencia, ll.enviado_em, a.data as ag_data, a.status as ag_status
            FROM lembrete_log ll LEFT JOIN agendamentos a ON a.id=ll.ag_id
            WHERE ll.salon_id=%s AND ll.ag_id IN (
                SELECT id FROM agendamentos WHERE salon_id=%s AND cli_id=%s
            ) ORDER BY ll.enviado_em DESC LIMIT 20""", (sid, sid, c['id']), 'all')
        out.append({
            'cliente': c,
            'agendamentos': [dict(a) for a in (ags or [])],
            'logs_lembrete': [dict(l) for l in (logs or [])]
        })
    return jsonify(out)

@app.route('/api/diagnostico-geral', methods=['GET'])
def diagnostico_geral():
    """Diagnóstico mestre: mostra a versão do backend e TODAS as fontes de envio ativas."""
    sid, err = require_salon()
    if err: return err
    BACKEND_VERSION = 124  # incrementado a cada deploy para conferir o que está no ar
    # Contato automático
    cfgs = db_exec("SELECT tipo, ativo, horario FROM contato_auto_config WHERE salon_id=%s", (sid,), 'all')
    # Campanhas ativas
    camps = db_exec("""SELECT id, titulo, status FROM disparo_campanha
        WHERE salon_id=%s AND status IN ('rodando','pausada_limite')""", (sid,), 'all')
    # Pós-atendimento
    pos = db_exec("SELECT COUNT(*) as n FROM servicos WHERE salon_id=%s AND pos_ativo=1", (sid,), 'one')
    # Envios reais nos últimos 2 dias
    envios = db_exec("""SELECT tipo, COUNT(*) as n, MAX(enviado_em) as ultimo
        FROM contato_auto_log WHERE salon_id=%s AND enviado_em > NOW() - INTERVAL '2 days'
        GROUP BY tipo""", (sid,), 'all')
    # Verifica se a TRAVA anti-loop (v122) está neste backend
    trava_v122 = True  # esta linha só existe no código v122+
    return jsonify({
        'backend_version': BACKEND_VERSION,
        'trava_antiloop_v122_presente': trava_v122,
        'contato_auto': [dict(c) for c in (cfgs or [])],
        'campanhas_ativas': [dict(c) for c in (camps or [])],
        'servicos_com_pos': pos['n'] if pos else 0,
        'envios_ultimos_2dias': [dict(e) for e in (envios or [])]
    })

@app.route('/api/comissoes/diagnostico', methods=['GET'])
def comissoes_diag():
    """Mostra os dados crus de cada comissão para entender o Valor Serv. zerado."""
    sid, err = require_salon()
    if err: return err
...
def comissoes_list():
    ...
    q = """SELECT c.*,a.data,a.h_ini,a.pag,
        CAST(COALESCE(NULLIF(a.preco,0), s.preco, 0) AS FLOAT) as svc_preco,
    ...
    for r in rows:
        d = dict(r)
        # Fallback: se não há preço de serviço mas há comissão e %, calcula o valor base
        if (not d.get('svc_preco')) and d.get('valor') and d.get('com_pct'):
            try:
                if float(d['com_pct']) > 0:
                    d['svc_preco'] = round(float(d['valor']) / (float(d['com_pct'])/100.0), 2)
            except Exception:
                pass
        out.append(d)
    return jsonify(out)
...
def emergencia_parar_tudo():
    """PARADA DE EMERGÊNCIA: desliga TUDO que envia mensagem automática.
    - Desativa contato automático (inativo, retorno, lembrete)
    - Cancela todas as campanhas de disparo (rodando/pausadas) e esvazia a fila
    Pode ser chamada logada OU com ?key= (cron_key) para acesso rápido."""
    key = request.args.get('key', '')
    keyrow = db_exec("SELECT valor FROM sistema_global WHERE chave='cron_key'", fetch='one')
    cron_key = keyrow['valor'] if keyrow else 'musa_cron_2024'
    if key and key == cron_key:
        # Para TODOS os salões (emergência global)
        db_exec("UPDATE contato_auto_config SET ativo=0")
        db_exec("UPDATE disparo_campanha SET status='cancelada' WHERE status IN ('rodando','pausada_limite')")
        db_exec("DELETE FROM disparo_log WHERE status='pendente'")
        db_commit()
        return jsonify({'ok': True, 'escopo': 'global', 'msg': 'Todos os envios automáticos foram parados.'})
    sid, err = require_salon()
    if err: return err
    ...
    db_commit()
    return jsonify({'ok': True, 'escopo': 'salao', 'msg': 'Todos os envios automáticos do seu salão foram parados.'})
...
@app.route('/api/retorno-alertas/diagnostico', methods=['GET'])
def retorno_alertas_diag():
    """Mostra todos os alertas de retorno e seus estados (para entender lista vazia)."""
    sid, err = require_salon()
    if err: return err
    total = db_exec("SELECT COUNT(*) as n FROM retorno_alertas WHERE salon_id=%s", (sid,), 'one')
    pendentes = db_exec("SELECT COUNT(*) as n FROM retorno_alertas WHERE salon_id=%s AND realizado=0", (sid,), 'one')
    realizados = db_exec("SELECT COUNT(*) as n FROM retorno_alertas WHERE salon_id=%s AND realizado=1", (sid,), 'one')
    hoje = today_br().isoformat()
    futuros = db_exec("SELECT COUNT(*) as n FROM retorno_alertas WHERE salon_id=%s AND realizado=0 AND data_retorno>%s", (sid, hoje), 'one')
    vencidos = db_exec("SELECT COUNT(*) as n FROM retorno_alertas WHERE salon_id=%s AND realizado=0 AND data_retorno<=%s", (sid, hoje), 'one')
    # Amostra das próximas datas
    amostra = db_exec("""SELECT ra.data_retorno, ra.svc_nome, ra.realizado, c.nome as cli_nome
        FROM retorno_alertas ra JOIN clientes c ON c.id=ra.cli_id
        WHERE ra.salon_id=%s ORDER BY ra.data_retorno DESC LIMIT 10""", (sid,), 'all')
    return jsonify({
        'hoje': hoje,
        'total_alertas': total['n'] if total else 0,
        'pendentes': pendentes['n'] if pendentes else 0,
        'realizados': realizados['n'] if realizados else 0,
        'pendentes_futuros': futuros['n'] if futuros else 0,
        'pendentes_vencidos_ou_hoje': vencidos['n'] if vencidos else 0,
        'amostra': [dict(r) for r in (amostra or [])]
    })

@app.route('/api/envios/diagnostico', methods=['GET'])
def envios_diag():
    """Mostra os últimos envios reais registrados (contato_auto_log) e campanhas de disparo."""
    sid, err = require_salon()
    if err: return err
    # Contagem por tipo nos últimos 3 dias
    por_tipo = db_exec("""SELECT tipo, COUNT(*) as n, MAX(enviado_em) as ultimo
        FROM contato_auto_log WHERE salon_id=%s AND enviado_em > NOW() - INTERVAL '3 days'
        GROUP BY tipo ORDER BY ultimo DESC""", (sid,), 'all')
    # Últimos 15 envios detalhados
    ultimos = db_exec("""SELECT cal.tipo, cal.enviado_em, c.nome as cli_nome
        ...
        WHERE cal.salon_id=%s ORDER BY cal.enviado_em DESC LIMIT 15""", (sid,), 'all')
    # Campanhas de disparo em massa
    camps = db_exec("""SELECT id, titulo, status, total, enviados, falhas, criado_em
        FROM disparo_campanha WHERE salon_id=%s ORDER BY criado_em DESC LIMIT 5""", (sid,), 'all')
    return jsonify({
        'por_tipo_3dias': [dict(r) for r in (por_tipo or [])],
        'ultimos_envios': [dict(r) for r in (ultimos or [])],
        'campanhas_disparo': [dict(r) for r in (camps or [])]
    })
...
def clientes_inativos():
    ...
    # Data de referência de cada cliente:
    #  - se tem ultima_visita (histórico real), usa ela
    #  - senão, usa criado_em (data de cadastro no sistema)
    # Cliente é inativo se essa data de referência for <= corte.
    # Quem foi cadastrado recentemente e nunca veio NÃO é inativo ainda (conta a partir do cadastro).
    rows = db_exec("""SELECT c.*,
        ...
        COALESCE(NULLIF(c.ultima_visita,''), NULLIF(c.criado_em,'')) as data_referencia
        FROM clientes c
        WHERE c.salon_id=%s AND c.ativo=1
          AND COALESCE(NULLIF(c.ultima_visita,''), NULLIF(c.criado_em,''), '1900-01-01') <= %s
        ORDER BY data_referencia ASC NULLS FIRST""", (sid,sid,sid,sid,sid,corte), 'all')
    return jsonify([dict(r) for r in rows])

@app.route('/api/clientes/recalcular-visitas', methods=['POST'])
def clientes_recalcular_visitas():
    """Recalcula a última visita de cada cliente com base no histórico real de atendimentos.
    Para quem não tem nenhum atendimento, garante uma data de cadastro (criado_em)
    para que a contagem de inatividade comece a partir de agora, não retroativa."""
    sid, err = require_salon()
    if err: return err
    # 1) Atualiza ultima_visita pelo atendimento concluído mais recente de cada cliente
    db_exec("""
    ...
    """, (sid, sid))
    # 2) Para clientes SEM atendimento e SEM criado_em, define criado_em = hoje
    #    (assim a inatividade conta a partir de agora, não imediatamente)
    hoje = today_br().isoformat()
    db_exec("""
...
def indisponibilidades():
    ...
    if request.method == 'GET':
        ...
        for r in rows:
            d = dict(r)
            # data_inicio/data_fim guardam "YYYY-MM-DD HH:MM" (ou só a data)
            ...
            d['h_ini'] = d_hini
            d['h_fim'] = d_hfim
            # Se foi pedido um dia específico, filtra os que tocam esse dia
            if data_f:
...
def disparo_preview():
    """Retorna a lista de clientes que receberiam o disparo, sem enviar nada."""
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
...
def disparo_processar():
    ...
    if key and key == cron_key:
        # Modo cron: processa um lote de cada campanha ativa.
        # SEGURANÇA: só processa campanhas criadas nas últimas 24h, para evitar
        # que campanhas antigas/esquecidas ("zumbis") fiquem disparando sozinhas.
        camps = db_exec("""SELECT id, salon_id FROM disparo_campanha
            WHERE status='rodando' AND criado_em > NOW() - INTERVAL '24 hours'""", fetch='all')
        # Campanhas 'rodando' com mais de 24h são automaticamente pausadas (não disparam mais)
...
def disparo_cancelar_tudo():
    """Cancela TODAS as campanhas em andamento (rodando/pausadas) do salão.
    Também remove os pendentes da fila, para garantir que nada mais seja enviado."""
    sid, err = require_salon()
    if err: return err
    # Quais campanhas estão ativas
    ativas = db_exec("""SELECT id FROM disparo_campanha
        WHERE salon_id=%s AND status IN ('rodando','pausada_limite')""", (sid,), 'all')
    ids = [r['id'] for r in (ativas or [])]
    db_exec("""UPDATE disparo_campanha SET status='cancelada'
        WHERE salon_id=%s AND status IN ('rodando','pausada_limite')""", (sid,))
    # Remove os pendentes dessas campanhas (zera a fila)
    if ids:
        db_exec("""DELETE FROM disparo_log WHERE salon_id=%s AND status='pendente'
            AND campanha_id = ANY(%s)""", (sid, ids))
    db_commit()
...
def programados_listar():
    """Central de mensagens programadas: tudo que pode disparar automaticamente.
    Reúne contato automático (inativo/retorno/lembrete), campanhas de disparo e pós-atendimento."""
    sid, err = require_salon()
    if err: return err
    itens = []

    # 1) Contato automático (inativo, retorno, lembrete)
    ...
    for c in (cfgs or []):
        ...
        itens.append({
            'grupo': 'automatico',
            'tipo': c['tipo'],
            'titulo': rotulos.get(c['tipo'], c['tipo']),
            'ativo': c.get('ativo', 0),
    ...
    for c in (camps or []):
        ...
        itens.append({
            'grupo': 'campanha',
            'id': c['id'],
            'titulo': 'Campanha: ' + (c.get('titulo') or 'Disparo'),
            'ativo': 1,
            'detalhe': c['status'] + ' · ' + str(c.get('enviados',0)) + '/' + str(c.get('total',0)) + ' enviados',
            'pode_excluir': True
        })

    # 3) Pós-atendimento (serviços com pos_ativo=1)
    servs = db_exec("SELECT id, nome FROM servicos WHERE salon_id=%s AND pos_ativo=1 AND ativo=1", (sid,), 'all')
    for s in (servs or []):
        s = dict(s)
        itens.append({
            'grupo': 'pos_atendimento',
            'id': s['id'],
            'titulo': 'Pós-atendimento: ' + s['nome'],
            'ativo': 1,
            'detalhe': '2h após concluir este serviço',
            'pode_excluir': True
        })
...
def programados_excluir():
    """Exclui/desativa um item programado específico."""
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    grupo = d.get('grupo')
...
def programados_cancelar_tudo():
    """Cancela/desativa TODOS os envios programados do salão."""
    sid, err = require_salon()
    if err: return err
    db_exec("UPDATE contato_auto_config SET ativo=0 WHERE salon_id=%s", (sid,))
    db_exec("UPDATE disparo_campanha SET status='cancelada' WHERE salon_id=%s AND status IN ('rodando','pausada_limite')", (sid,))
    ...
    for s in sids:
        ...
        for cfg in (cfgs or []):
            ...
            if tipo == 'retorno':
                ...
                # O lembrete tem rota própria (/api/lembrete/disparar). NUNCA processar aqui,
                # para nunca puxar clientes inativos por engano e mandar texto de lembrete errado.
                clientes = []
            elif tipo == 'inativo':
                ...
                # Assim, quem foi importado sem histórico só é contatado depois do prazo
                # contado a partir do cadastro — nunca retroativamente.
                clientes = db_exec("""
            ...
            for cli in (clientes or []):
                ...
                if jaenv:
                    diag['ja_contatado'] += 1
                    continue
                # TRAVA EXTRA (anti-loop diário): se JÁ existe QUALQUER registro deste cliente+tipo
                # nas últimas 20h, não reenvia. Protege contra reenvio repetido às 8h todo dia
                # mesmo que a janela acima falhe por algum motivo.
                hoje_check = db_exec("""SELECT id FROM contato_auto_log
                    WHERE salon_id=%s AND tipo=%s AND cli_id=%s AND enviado_em > NOW() - INTERVAL '20 hours'""",
                    (s, tipo, cli['cli_id']), 'one')
                if hoje_check:
                    diag['ja_contatado'] += 1
                    continue
                # Registra ANTES de enviar (evita reenvio se o envio demorar/duplicar a chamada)
    ...
    for s in sids:
        ...
        for rotulo, mins_antes in janelas:
            ...
            for ag in (ags or []):
                ...
                if ja_dia:
                    # Registra este ag_id também para não processar nas próximas rodadas
...
def wpp_webhook(sid):
    ...
    # ── ANTI-DUPLICAÇÃO ──
    # A Evolution às vezes chama o webhook mais de uma vez para a mesma mensagem,
    # o que faz a IA responder em duplicidade ("confusão de mensagens").
    # Guardamos o ID da mensagem; se já foi processado, ignoramos.
    try:
        msg_id = (msg_data.get('key', {}) or {}).get('id', '')
        if msg_id:
            chave_msg = 'msgproc_' + str(sid) + '_' + msg_id[:40]
            ja = db_exec("SELECT valor FROM sistema_global WHERE chave=%s", (chave_msg,), 'one')
            if ja:
                return jsonify({'ok': True, 'duplicada': True})
            db_exec("""INSERT INTO sistema_global (chave, valor) VALUES (%s, NOW()::text)
                       ON CONFLICT (chave) DO NOTHING""", (chave_msg,))
            db_commit()
    except Exception:
        pass
    ...
    # CAMPANHA COM IA DESATIVADA: se este cliente recebeu um disparo de uma campanha
    # marcada com ia_ativa=0, a IA NÃO responde (você cuida manualmente das respostas).
    try:
        camp_ia = db_exec("""SELECT dc.ia_ativa FROM disparo_log dl
            JOIN disparo_campanha dc ON dc.id=dl.campanha_id
            WHERE dl.salon_id=%s AND dl.status='enviado'
              AND regexp_replace(dl.numero,'[^0-9]','','g') LIKE %s
            ORDER BY dl.enviado_em DESC LIMIT 1""",
            (sid, '%' + numero_cliente[-8:]), 'one')
        if camp_ia and camp_ia.get('ia_ativa') == 0:
            return jsonify({'ok': True, 'ia_desativada_campanha': True})
    except Exception:
        pass
    pros   = db_exec("SELECT id,nome,cargo,ativo FROM profissionais WHERE salon_id=%s AND ativo=1", (sid,), 'all')
    svcs   = db_exec("SELECT id,nome,preco,duracao_min,ativo FROM servicos WHERE salon_id=%s AND ativo=1", (sid,), 'all')
