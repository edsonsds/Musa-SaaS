...
def _disparo_worker(sid, campanha_id, mensagem, destinatarios, limite_dia):
    """Processa o envio em lote de forma responsável, em segundo plano.
    - Espaça os envios (intervalo natural entre mensagens) para não parecer rajada de spam.
    - Respeita o limite diário: ao atingir, pausa a campanha (retoma no dia seguinte).
    - Registra cada envio para nunca repetir o mesmo cliente.
    Importante: isto NÃO burla detecção. Só evita rajadas e respeita limites — uso responsável."""
    import time, random
    enviados_hoje = 0
    for dest in destinatarios:
        # Verifica se a campanha foi cancelada
        camp = db_exec("SELECT status FROM disparo_campanha WHERE id=%s AND salon_id=%s", (campanha_id, sid), 'one')
        if not camp or camp['status'] == 'cancelada':
            break
        # Limite diário atingido → pausa
        if enviados_hoje >= limite_dia:
            db_exec("UPDATE disparo_campanha SET status='pausada_limite' WHERE id=%s", (campanha_id,))
            db_commit()
            break

        cli_id = dest.get('cli_id')
        numero = dest.get('numero', '')
        nome = dest.get('nome', '')
        # Pula quem já recebeu (registro único por campanha+cliente)
        ja = db_exec("SELECT status FROM disparo_log WHERE campanha_id=%s AND cli_id=%s", (campanha_id, cli_id), 'one')
        if ja and ja['status'] == 'enviado':
            continue

        # Personaliza com o primeiro nome (mensagem mais natural e individual)
        primeiro = (nome or '').split(' ')[0] if nome else ''
        texto = mensagem.replace('{nome}', primeiro) if '{nome}' in mensagem else mensagem
        ...
        db_exec("""INSERT INTO disparo_log (salon_id,campanha_id,cli_id,numero,status,enviado_em)
                   VALUES (%s,%s,%s,%s,%s,NOW())
                   ON CONFLICT (campanha_id,cli_id) DO UPDATE SET status=EXCLUDED.status, enviado_em=NOW()""",
                (sid, campanha_id, cli_id, numero, 'enviado' if ok else 'falha'))
        if ok:
        ...
        # Intervalo natural entre mensagens (30 a 90 segundos) — evita rajada
        time.sleep(random.randint(30, 90))

    # Finaliza se enviou tudo
    camp = db_exec("SELECT total, enviados, falhas, status FROM disparo_campanha WHERE id=%s", (campanha_id,), 'one')
    if camp and camp['status'] == 'rodando' and (camp['enviados'] + camp['falhas']) >= camp['total']:
...
def disparo_iniciar():
    ...
    filtro = d.get('filtro', 'todos')  # 'todos', 'ativos_90', 'ativos_180'
    if len(mensagem) < 5:
        return jsonify({'ok': False, 'erro': 'Escreva a mensagem que será enviada.'})
    if limite_dia > 200:
        limite_dia = 200  # teto de segurança

    # Selecionar destinatários conforme o filtro (priorizando clientes recentes/ativos)
    if filtro == 'ativos_90':
        clientes = db_exec("""SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1
                           ...
                           (sid, (today_br() - datetime.timedelta(days=90)).isoformat()), 'all')
    elif filtro == 'ativos_180':
        clientes = db_exec("""SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1
                              AND ultima_visita >= %s ORDER BY ultima_visita DESC""",
                           (sid, (today_br() - datetime.timedelta(days=180)).isoformat()), 'all')
    else:
        clientes = db_exec("""SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1
                              ORDER BY ultima_visita DESC NULLS LAST""", (sid,), 'all')

    destinatarios = [{'cli_id': c['id'], 'nome': c['nome'], 'numero': c['tel']} for c in (clientes or [])]
    ...
    camp = db_exec("""INSERT INTO disparo_campanha (salon_id,titulo,mensagem,total,limite_dia,status)
                   ...
                   (sid, titulo, mensagem, len(destinatarios), limite_dia), 'one')
    db_commit()
    campanha_id = camp['id']

    import threading
    t = threading.Thread(target=_disparo_worker, args=(sid, campanha_id, mensagem, destinatarios, limite_dia), daemon=True)
    t.start()

    return jsonify({'ok': True, 'campanha_id': campanha_id, 'total': len(destinatarios),
                    'msg': 'Disparo iniciado! Enviando de forma espaçada (1 a cada 30-90s), respeitando o limite de ' + str(limite_dia) + '/dia.'})
...
def disparo_retomar(campanha_id):
    """Retoma uma campanha pausada por limite diário — envia para quem ainda não recebeu."""
    sid, err = require_salon()
    if err: return err
    camp = db_exec("SELECT * FROM disparo_campanha WHERE id=%s AND salon_id=%s", (campanha_id, sid), 'one')
    if not camp:
        return jsonify({'ok': False, 'erro': 'Campanha não encontrada'})
    # Pegar quem ainda não recebeu
    pendentes = db_exec("""SELECT c.id,c.nome,c.tel FROM clientes c
        WHERE c.salon_id=%s AND c.tel!='' AND c.ativo=1
        AND c.id NOT IN (SELECT cli_id FROM disparo_log WHERE campanha_id=%s AND status='enviado')
        ORDER BY c.ultima_visita DESC NULLS LAST""", (sid, campanha_id), 'all')
    destinatarios = [{'cli_id': c['id'], 'nome': c['nome'], 'numero': c['tel']} for c in (pendentes or [])]
    if not destinatarios:
        db_exec("UPDATE disparo_campanha SET status='concluida' WHERE id=%s", (campanha_id,))
        db_commit()
    ...
    db_commit()
    import threading
    t = threading.Thread(target=_disparo_worker, args=(sid, campanha_id, camp['mensagem'], destinatarios, camp['limite_dia']), daemon=True)
    t.start()
    return jsonify({'ok': True, 'msg': 'Retomado! Enviando para os ' + str(len(destinatarios)) + ' restantes.'})
