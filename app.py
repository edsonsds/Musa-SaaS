...
def wpp_webhook(sid):
    ...
    if '@g.us' in remote_jid:
        return jsonify({'ok': True})
    # O remoteJid pode vir como @lid (identificador interno). O número real está em 'sender'.
    sender = msg_data.get('sender', '') or data.get('sender', '')
    if '@lid' in remote_jid and sender:
        numero_cliente = sender.replace('@s.whatsapp.net', '').replace('@c.us', '')
    else:
        numero_cliente = remote_jid.replace('@s.whatsapp.net', '').replace('@c.us', '').replace('@lid', '')
    # Remover sufixo de device (ex: :12@) se houver
    numero_cliente = numero_cliente.split(':')[0].split('@')[0]
        ...
        for pl in tentativas:
            try:
                ...
                with _ur.urlopen(req, timeout=15) as resp:
                    ...
                    log.append('OK ' + str(resp.status) + ': ' + body[:120])
