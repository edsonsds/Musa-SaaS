...
def servico_lembrete_semanal(svid):
    """Ativa/desativa o lembrete semanal para um serviço."""
    sid, err = require_salon()
    if err: return err
    ativo = 1 if (request.json or {}).get('ativo') else 0
    db_exec("UPDATE servicos SET lembrete_semanal=%s WHERE id=%s AND salon_id=%s", (ativo, svid, sid))
    db_commit()
    return jsonify({'ok': True})
...
def lembrete_semanal_disparar():
    ...
    total = 0
    for s in sids:
        # Serviços com lembrete ativo neste salão
        svcs_lem = db_exec(
            "SELECT id, nome FROM servicos WHERE salon_id=%s AND lembrete_semanal=1 AND ativo=1",
            (s,), 'all')
        if not svcs_lem: continue
        svc_ids = [r['id'] for r in svcs_lem]
        svc_nomes = ', '.join(r['nome'] for r in svcs_lem)
        ini_sem = (hoje - _dt.timedelta(days=hoje.weekday())).isoformat()
        fim_sem = (hoje + _dt.timedelta(days=6 - hoje.weekday())).isoformat()
        clientes = db_exec("""
            ...
            JOIN agendamentos a ON a.cli_id=c.id AND a.salon_id=c.salon_id
            WHERE c.salon_id=%s AND c.ativo=1 AND c.tel!=''
              AND a.svc_id = ANY(%s) AND a.status='concluido'
              AND a.data >= %s
              ...
              AND NOT EXISTS (SELECT 1 FROM lembrete_semanal_log l
                              WHERE l.salon_id=%s AND l.cli_id=c.id
                                AND l.enviado_em >= NOW() - INTERVAL '6 days')
        """, (s, svc_ids, (hoje - _dt.timedelta(days=60)).isoformat(),
        ...
        for cli in clientes:
            ...
            msg = ('Oi ' + primeiro + '! 💅 Tudo bem?\n\n'
                   'Passando para saber se você gostaria de agendar seu ' + svc_nomes +
