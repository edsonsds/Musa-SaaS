...
def lembrete_semanal_disparar():
    ...
    # Manual (botão): roda direto e retorna o resultado
    try:
        msg_override = (request.json or {}).get('msg_fixa_override', '') if request.method == 'POST' else ''
        total = _run_lembrete_semanal(sids, hoje, dia_semana, True, msg_override)
        return jsonify({'ok': True, 'enviados': total})
    except Exception as ex:
        import traceback
        tb = traceback.format_exc()
        try:
            db_exec("""INSERT INTO sistema_global (chave, valor) VALUES ('last_lem_err', %s)
                       ON CONFLICT (chave) DO UPDATE SET valor=EXCLUDED.valor""", (str(ex)[:300] + ' | ' + tb[-400:],))
            db_commit()
        except Exception: pass
        return jsonify({'ok': False, 'erro': str(ex), 'detalhe': tb[-500:]}), 500
