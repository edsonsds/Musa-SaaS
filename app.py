...
def _gerar_msg_lembrete_semanal(sid, svc, primeiro_nome, nome_salao, cfg_ia):
    ...
    _frases_base = [
        'Oi {nome}! 💅 Vai fazer {svc} essa semana? Me fala que já reservo seu horário!',
        'Ei {nome}! Tá na hora do {svc} né? 💅 Quer que eu separe um horário pra você?',
        '{nome}, a semana tá passando! 😄 Bora garantir seu {svc}?',
        'Oi {nome}! Passa aqui essa semana pro seu {svc}? 💅 Só me confirmar!',
        'E aí {nome}! Vai deixar o {svc} pra lá? 😂 Me chama que a gente acha um horário!',
        'Oi {nome}! Lembrando do seu {svc} essa semana aqui no {salao} 💅 Bora?',
        '{nome}! Semana boa pra fazer {svc}! Me manda mensagem que já marco pra você 😊',
        'Oi {nome}! Ainda dá tempo de encaixar seu {svc} essa semana 💅 Quer?',
    ]
    ...
    if modo == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            import requests as _rq
            # Sorteia uma estrutura de frase diferente a cada vez para forçar variação
            estilos = [
                'Comece com "Oi" e faça uma pergunta direta sobre agendar.',
                'Comece com o nome da cliente e use uma frase bem curta e animada.',
                'Use um emoji de surpresa ou alegria e termine com uma pergunta.',
                'Seja bem direta: diga que tem horário disponível e pergunte se ela quer.',
                'Use uma frase informal como "E aí" ou "Bora" para soar natural.',
            ]
            estilo = _random.choice(estilos)
            prompt = (
                'Você é a recepcionista de um salão de beleza brasileiro chamado ' + nome_salao + '. '
                'Mande uma mensagem curta de WhatsApp para a cliente ' + primeiro_nome + ' '
                'lembrando que está na hora de agendar o serviço de ' + svc['nome'] + '. '
                'Estilo desta mensagem: ' + estilo + ' '
                'REGRAS: português do Brasil informal, máximo 2 linhas, termine com convite para remarcar, '
                '1 emoji no máximo, sem linguagem formal, sem "saudade", sem "prezada". '
...
def _run_lembrete_semanal(sids, hoje, dia_semana, forcar, msg_override=''):
    ...
    for s in sids:
        ...
        ini_sem = (hoje - _dt.timedelta(days=hoje.weekday())).isoformat()
        fim_sem = (hoje + _dt.timedelta(days=6 - hoje.weekday())).isoformat()
        salao = db_exec("SELECT nome FROM saloes WHERE id=%s", (s,), 'one')
        nome_salao = salao['nome'] if salao else 'nosso salão'
        cfg_ia = db_exec("SELECT groq_key FROM wpp_ia_config WHERE salon_id=%s", (s,), 'one')
        ...
        for svc in (svcs_lem or []):
            ...
            for cli in clientes:
                ...
                if _enviar_wpp(s, cli['tel'], msg):
                    db_exec("""INSERT INTO lembrete_semanal_log (salon_id, cli_id)
                               VALUES (%s, %s) ON CONFLICT DO NOTHING""", (s, cli['id']))
                    db_commit()
                    total += 1
