...
def montar_sistema(salao, profissionais, servicos, historico_cliente=None, personalidade='', em_andamento=False):
    ...
    if (salao or {}).get('_cliente_novo'):
        bloco_cliente = "\nCLIENTE NOVA (sem cadastro): pergunte o nome completo dela uma vez, de forma natural, durante a conversa. Use esse nome no ##AGENDAR##.\n"
    elif (salao or {}).get('_cliente_nome'):
        bloco_cliente = "\nCLIENTE: " + str((salao or {}).get('_cliente_nome')) + " (já cadastrada). Trate pelo primeiro nome.\n"
    else:
        bloco_cliente = ''
