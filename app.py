...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS servicos (
        ...
        pos_ativo INTEGER DEFAULT 0,
        pos_texto TEXT DEFAULT '',
        lembrete_semanal INTEGER DEFAULT 0,
        lembrete_semanal_modo TEXT DEFAULT 'fixo',
        lembrete_semanal_texto TEXT DEFAULT ''
    )""")
    try:
        cur.execute("ALTER TABLE servicos ADD COLUMN IF NOT EXISTS pos_ativo INTEGER DEFAULT 0")
        cur.execute("ALTER TABLE servicos ADD COLUMN IF NOT EXISTS pos_texto TEXT DEFAULT ''")
        cur.execute("ALTER TABLE servicos ADD COLUMN IF NOT EXISTS lembrete_semanal INTEGER DEFAULT 0")
        cur.execute("ALTER TABLE servicos ADD COLUMN IF NOT EXISTS lembrete_semanal_modo TEXT DEFAULT 'fixo'")
        cur.execute("ALTER TABLE servicos ADD COLUMN IF NOT EXISTS lembrete_semanal_texto TEXT DEFAULT ''")
    except Exception:
        pass
    ...
    try:
        cur.execute("ALTER TABLE agendamentos ADD COLUMN IF NOT EXISTS concluido_em TIMESTAMP")
    except Exception:
        pass
    ...
    CREATE TABLE IF NOT EXISTS despesas (
        ...
        eh_adiantamento INTEGER DEFAULT 0,
        status_reconhecimento TEXT DEFAULT 'pendente'
    )""")
    try:
        cur.execute("ALTER TABLE despesas ADD COLUMN IF NOT EXISTS pro_id INTEGER DEFAULT 0")
    except Exception: conn.rollback()
    try:
        cur.execute("ALTER TABLE despesas ADD COLUMN IF NOT EXISTS eh_adiantamento INTEGER DEFAULT 0")
    except Exception: conn.rollback()
    try:
        cur.execute("ALTER TABLE despesas ADD COLUMN IF NOT EXISTS status_reconhecimento TEXT DEFAULT 'pendente'")
    except Exception: conn.rollback()
    ...
    try:
        cur.execute("ALTER TABLE disparo_campanha ADD COLUMN IF NOT EXISTS ia_ativa INTEGER DEFAULT 1")
    except Exception:
        pass

    # Pós-atendimento (cuidados pós-procedimento)
    cur.execute("""
    CREATE TABLE IF NOT EXISTS pos_atend_log (
        id SERIAL PRIMARY KEY,
        salon_id INTEGER NOT NULL,
        ag_id INTEGER NOT NULL,
        enviado_em TIMESTAMP DEFAULT NOW(),
    ...
    # Lembrete semanal (manicure/serviços específicos)
    cur.execute("""
    CREATE TABLE IF NOT EXISTS lembrete_semanal_log (
        id SERIAL PRIMARY KEY,
        salon_id INTEGER NOT NULL,
        cli_id INTEGER NOT NULL,
        enviado_em TIMESTAMP DEFAULT NOW()
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS lembrete_semanal_bloqueio (
        id SERIAL PRIMARY KEY,
        salon_id INTEGER NOT NULL,
        cli_id INTEGER NOT NULL,
        motivo TEXT DEFAULT '',
        criado_em TIMESTAMP DEFAULT NOW(),
        UNIQUE(salon_id, cli_id)
    )""")
...
def diag_minha_agenda():
    ...
    do profissional logado, sem filtro de data, para comparar."""
    if session.get('uperfil') != 'profissional' or not session.get('pro_id'):
        return jsonify({'erro': 'Acesso negado'}), 403
    sid    = session['salon_id']
    pro_id = session['pro_id']
    hoje = today_br()
    todos = db_exec("""SELECT a.id, a.data, a.h_ini, a.status, c.nome as cliente
        FROM agendamentos a LEFT JOIN clientes c ON c.id=a.cli_id
        WHERE a.salon_id=%s AND a.pro_id=%s
        ORDER BY a.data DESC LIMIT 15""", (sid, pro_id), 'all')
    return jsonify({
        'pro_id': pro_id,
        'salon_id': sid,
        'hoje_calculado_servidor': hoje.isoformat(),
        'agendamentos_recentes_deste_profissional': [dict(r) for r in (todos or [])]
    })
...
def minha_comissao():
    ...
    pro_id = session['pro_id']
    hoje   = today_br()
    ini    = request.args.get('ini', hoje.replace(day=1).isoformat())
    ...
    adto   = db_exec("""SELECT COALESCE(SUM(valor),0) as t FROM despesas
        WHERE salon_id=%s AND pro_id=%s AND eh_adiantamento=1 AND status_reconhecimento!='contestado'
          AND data BETWEEN %s AND %s""", (sid,pro_id,ini,fim), 'one')
    total_com = float(total['t'] or 0)
    total_adto = float(adto['t'] or 0)
    return jsonify({
        'pro_id':pro_id,'periodo_ini':ini,'periodo_fim':fim,
        'total_comissao':total_com,'qtd_atendimentos':int(qtd['n'] or 0),
        'total_adiantamentos':total_adto,
        'total_liquido': round(total_com - total_adto, 2)
    })

@app.route('/api/profissional/adiantamentos-pendentes', methods=['GET'])
def adiantamentos_pendentes():
    """Lista os adiantamentos do profissional logado que ainda não foram reconhecidos/contestados."""
    if session.get('uperfil') != 'profissional' or not session.get('pro_id'):
        return jsonify({'erro': 'Acesso negado'}), 403
    sid    = session['salon_id']
    pro_id = session['pro_id']
    rows = db_exec("""SELECT id, data, descricao, valor FROM despesas
        WHERE salon_id=%s AND pro_id=%s AND eh_adiantamento=1 AND status_reconhecimento='pendente'
        ORDER BY data DESC""", (sid, pro_id), 'all')
    return jsonify([dict(r) for r in (rows or [])])

@app.route('/api/profissional/adiantamento/<int:did>/responder', methods=['POST'])
def adiantamento_responder(did):
    """O profissional reconhece ou contesta um adiantamento lançado."""
    if session.get('uperfil') != 'profissional' or not session.get('pro_id'):
        return jsonify({'erro': 'Acesso negado'}), 403
    sid    = session['salon_id']
    pro_id = session['pro_id']
    d = request.json or {}
    reconhece = d.get('reconhece', False)
    novo_status = 'reconhecido' if reconhece else 'contestado'
    db_exec("""UPDATE despesas SET status_reconhecimento=%s
               WHERE id=%s AND salon_id=%s AND pro_id=%s AND eh_adiantamento=1""",
            (novo_status, did, sid, pro_id))
    db_commit()
    return jsonify({'ok': True, 'status': novo_status})

# Para o ADMIN ver/gerenciar adiantamentos contestados
@app.route('/api/adiantamentos/contestados', methods=['GET'])
def adiantamentos_contestados():
    sid, err = require_salon()
    if err: return err
    rows = db_exec("""SELECT d.*, p.nome as pro_nome FROM despesas d
        LEFT JOIN profissionais p ON p.id=d.pro_id
        WHERE d.salon_id=%s AND d.eh_adiantamento=1 AND d.status_reconhecimento='contestado'
        ORDER BY d.data DESC""", (sid,), 'all')
    return jsonify([dict(r) for r in (rows or [])])
...
@app.route('/api/servicos/<int:svid>/pos-atendimento', methods=['PUT'])
def servico_pos(svid):
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    ativo = 1 if d.get('pos_ativo') else 0
    texto = (d.get('pos_texto') or '').strip()
    db_exec("UPDATE servicos SET pos_ativo=%s, pos_texto=%s WHERE id=%s AND salon_id=%s",
            (ativo, texto, svid, sid))
    db_commit()
    return jsonify({'ok': True})

@app.route('/api/servicos/<int:svid>/pos-sugestao', methods=['POST'])
def servico_pos_sugestao(svid):
    sid, err = require_salon()
    if err: return err
    svc = db_exec("SELECT nome FROM servicos WHERE id=%s AND salon_id=%s", (svid, sid), 'one')
    if not svc:
        return jsonify({'ok': False, 'erro': 'Serviço não encontrado'})
    cfg = db_exec("SELECT groq_key FROM wpp_ia_config WHERE salon_id=%s", (sid,), 'one')
    nome_svc = svc['nome']
    if not cfg or not cfg.get('groq_key'):
        txt = ('Oi {nome}! 💕 Obrigado por escolher nosso salão para cuidar de você hoje. '
               'Após o seu ' + nome_svc + ', mantenha os cuidados recomendados pela sua profissional '
               'para preservar o resultado. Qualquer dúvida, é só chamar a gente. Até breve! ✨')
        return jsonify({'ok': True, 'texto': txt, 'fonte': 'padrao'})
    try:
        import requests as _rq
        prompt = ('Escreva uma mensagem curta e calorosa de WhatsApp para um cliente de salão de beleza, '
                  'enviada 2 horas após o procedimento "' + nome_svc + '". '
                  'Comece agradecendo por escolher o salão e dê 2 ou 3 dicas práticas de cuidado pós-procedimento '
                  'específicas para esse serviço. Use {nome} como espaço para o nome do cliente. '
                  'Máximo 4 linhas, tom acolhedor, com 1 ou 2 emojis. Responda só a mensagem, sem aspas.')
        r = _rq.post('https://api.groq.com/openai/v1/chat/completions',
            ...
            json={'model': 'llama-3.1-8b-instant', 'messages': [{'role':'user','content':prompt}],
                  'max_tokens': 250, 'temperature': 0.7}, timeout=20)
        if r.status_code == 200:
            txt = r.json()['choices'][0]['message']['content'].strip()
            return jsonify({'ok': True, 'texto': txt, 'fonte': 'ia'})
    except Exception:
        pass
    txt = ('Oi {nome}! 💕 Obrigado por escolher nosso salão. Após o seu ' + nome_svc +
           ', siga os cuidados recomendados para manter o resultado lindo. Qualquer dúvida, chame a gente! ✨')
    return jsonify({'ok': True, 'texto': txt, 'fonte': 'padrao'})

@app.route('/api/pos-atendimento/processar', methods=['GET','POST'])
def pos_atendimento_processar():
    ...
    if not (key and key == cron_key):
        sid_chk, err = require_salon()
        if err: return err
    enviados = 0
    pendentes = db_exec("""
        SELECT a.id as ag_id, a.salon_id, a.cli_id, a.svc_id,
               c.nome as cli_nome, c.tel as cli_tel,
               s.nome as svc_nome, s.pos_texto
        FROM agendamentos a
        JOIN servicos s ON s.id=a.svc_id AND s.salon_id=a.salon_id
        JOIN clientes c ON c.id=a.cli_id
        WHERE a.status='concluido' AND s.pos_ativo=1 AND s.pos_texto != ''
          AND c.tel != ''
          AND COALESCE(a.concluido_em, a.criado_em) <= NOW() - INTERVAL '2 hours'
          AND COALESCE(a.concluido_em, a.criado_em) >= NOW() - INTERVAL '6 hours'
          AND NOT EXISTS (SELECT 1 FROM pos_atend_log pl WHERE pl.ag_id=a.id)
        LIMIT 50
    """, fetch='all')
    for p in (pendentes or []):
        p = dict(p)
        primeiro = (p.get('cli_nome') or '').split(' ')[0]
        texto = p['pos_texto'].replace('{nome}', primeiro)
        ok = False
        try:
            ok = _enviar_wpp(p['salon_id'], p['cli_tel'], texto)
        except Exception:
            ok = False
        db_exec("INSERT INTO pos_atend_log (salon_id, ag_id) VALUES (%s,%s) ON CONFLICT (ag_id) DO NOTHING",
                (p['salon_id'], p['ag_id']))
        db_commit()
        if ok:
            enviados += 1
...
@app.route('/api/servicos/<int:svid>/lembrete-semanal', methods=['PUT'])
def servico_lembrete_semanal(svid):
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    ...
    db_exec("""UPDATE servicos SET lembrete_semanal=%s, lembrete_semanal_modo=%s, lembrete_semanal_texto=%s
               WHERE id=%s AND salon_id=%s""", (ativo, modo, texto, svid, sid))
    db_commit()
    return jsonify({'ok': True})

@app.route('/api/servicos/<int:svid>/lembrete-semanal-sugestao', methods=['POST'])
def servico_lembrete_semanal_sugestao(svid):
    sid, err = require_salon()
    if err: return err
    svc = db_exec("SELECT nome FROM servicos WHERE id=%s AND salon_id=%s", (svid, sid), 'one')
    if not svc:
    ...
    nome_svc = svc['nome']
    if not cfg or not cfg.get('groq_key'):
        txt = ('Oi {nome}! 💅 Tudo bem? Passando para saber se você gostaria de agendar seu ' +
               nome_svc + ' esta semana. Responda aqui para reservar seu horário! 💕')
        return jsonify({'ok': True, 'texto': txt, 'fonte': 'padrao'})
    try:
        import requests as _rq
        prompt = ('Escreva uma mensagem curta e simpática de WhatsApp convidando uma cliente de salão de beleza '
                  'a agendar o serviço "' + nome_svc + '" nesta semana. Use {nome} como espaço para o nome dela. '
                  'Tom leve, sem parecer repetitiva ou insistente. Máximo 3 linhas, 1 emoji. Sem aspas.')
        r = _rq.post('https://api.groq.com/openai/v1/chat/completions',
            headers={'Authorization': 'Bearer ' + cfg['groq_key'], 'Content-Type': 'application/json'},
            json={'model': 'llama-3.1-8b-instant', 'messages': [{'role':'user','content':prompt}],
        ...
        pass
    txt = ('Oi {nome}! 💅 Passando para saber se você gostaria de agendar seu ' + nome_svc +
           ' esta semana. Responda aqui para reservar! 💕')
    return jsonify({'ok': True, 'texto': txt, 'fonte': 'padrao'})

@app.route('/api/lembrete-semanal/lista', methods=['GET'])
def lembrete_semanal_lista():
    sid, err = require_salon()
    if err: return err
    import datetime as _dt
    hoje = today_br()
    ini_sem = (hoje - _dt.timedelta(days=hoje.weekday())).isoformat()
    fim_sem = (hoje + _dt.timedelta(days=6 - hoje.weekday())).isoformat()
    clientes = db_exec("""
        SELECT DISTINCT c.id, c.nome, c.tel, MAX(a.data) as ultima_visita
        FROM clientes c
        JOIN agendamentos a ON a.cli_id=c.id AND a.salon_id=c.salon_id
        JOIN servicos s ON s.id=a.svc_id AND s.salon_id=c.salon_id
        WHERE c.salon_id=%s AND c.ativo=1 AND c.tel!=''
          AND s.lembrete_semanal=1
          AND a.status='concluido'
          AND a.data >= %s
          AND NOT EXISTS (
              SELECT 1 FROM lembrete_semanal_bloqueio b
              WHERE b.salon_id=%s AND b.cli_id=c.id
          )
          AND NOT EXISTS (
              SELECT 1 FROM agendamentos ag2
              WHERE ag2.salon_id=%s AND ag2.cli_id=c.id
                AND ag2.data BETWEEN %s AND %s
                AND ag2.status NOT IN ('cancelado')
          )
          AND NOT EXISTS (
              SELECT 1 FROM lembrete_semanal_log l
              WHERE l.salon_id=%s AND l.cli_id=c.id
                AND l.enviado_em >= NOW() - INTERVAL '14 days'
          )
        GROUP BY c.id, c.nome, c.tel
        ORDER BY c.nome
    """, (sid, (hoje - _dt.timedelta(days=60)).isoformat(),
          sid, sid, ini_sem, fim_sem, sid), 'all')
    clientes = [dict(c) for c in (clientes or [])]
    terca  = [c for c in clientes if c['id'] % 2 == 0]
    quarta = [c for c in clientes if c['id'] % 2 != 0]
    return jsonify({'total': len(clientes), 'terca': terca, 'quarta': quarta})

@app.route('/api/lembrete-semanal/bloquear', methods=['POST'])
def lembrete_semanal_bloquear():
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    cli_id = d.get('cli_id')
    motivo = d.get('motivo', 'Não quer receber')
    db_exec("""INSERT INTO lembrete_semanal_bloqueio (salon_id, cli_id, motivo)
               VALUES (%s, %s, %s) ON CONFLICT (salon_id, cli_id) DO NOTHING""",
            (sid, cli_id, motivo))
    db_commit()
    return jsonify({'ok': True})

@app.route('/api/lembrete-semanal/desbloquear', methods=['POST'])
def lembrete_semanal_desbloquear():
    sid, err = require_salon()
    if err: return err
    cli_id = (request.json or {}).get('cli_id')
    db_exec("DELETE FROM lembrete_semanal_bloqueio WHERE salon_id=%s AND cli_id=%s", (sid, cli_id))
    db_commit()
    return jsonify({'ok': True})

def _gerar_msg_lembrete_semanal(sid, svc, primeiro_nome, nome_salao, cfg_ia):
    modo = svc.get('lembrete_semanal_modo') or 'fixo'
    texto_fixo = (svc.get('lembrete_semanal_texto') or '').strip()

    # Banco de frases rotativas — fallback humanizado mesmo sem IA
    _frases_base = [
        'Oi {nome}! 💅 Vai fazer {svc} essa semana aqui no {salao}? Me fala que já reservo seu horário!',
        'Ei {nome}! Tá na hora de cuidar das unhas né? 💅 Aqui no {salao} tem horário pra você!',
        '{nome}, a semana tá passando! 😄 Bora garantir seu {svc} aqui no {salao}?',
        'Oi {nome}! Passa aqui no {salao} essa semana pro seu {svc}? Só me confirmar! 💅',
        'E aí {nome}! Vai deixar as unhas pra lá? 😂 Me chama que a gente acha um horário no {salao}!',
        'Oi {nome}! Lembrando do seu {svc} aqui no {salao} essa semana 💅 Bora?',
        '{nome}! Semana boa pra fazer {svc} no {salao}! Me manda mensagem que já marco pra você 😊',
        'Oi {nome}! Ainda dá tempo de encaixar seu {svc} aqui no {salao} essa semana 💅 Quer?',
    ]
    import random as _random

    if modo == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            import requests as _rq
            estilos = [
                'Comece com "Oi" e faça uma pergunta direta e animada sobre vir fazer as unhas.',
                'Comece com o nome da cliente e use uma frase curta e descontraída sobre as unhas.',
                'Use "E aí" ou "Bora" e mencione que tem horário disponível para as unhas.',
                'Seja direta: diga que está passando para lembrar das unhas e pergunte se quer marcar.',
                'Use um toque de humor leve — tipo "deixar as unhas pra lá não dá né?" — e convide.',
            ]
            estilo = _random.choice(estilos)
            prompt = (
                'Você é a recepcionista do salão de beleza "' + nome_salao + '" no Brasil. '
                'Mande uma mensagem de WhatsApp para a cliente ' + primeiro_nome + ' '
                'lembrando que está na hora de vir fazer ' + svc['nome'] + ' (serviço de unhas — manicure/pedicure). '
                'A mensagem DEVE mencionar o nome do salão "' + nome_salao + '" para a cliente saber de onde vem. '
                'Estilo desta mensagem: ' + estilo + ' '
                'REGRAS: português do Brasil informal, máximo 2 linhas, '
                'termine com convite claro para agendar (ex: "Quer marcar?" ou "Me chama!"), '
                '1 emoji no máximo, sem linguagem formal, sem "prezada", sem "saudade". '
                'Responda APENAS com a mensagem, sem aspas, sem explicação.'
            )
            r = _rq.post('https://api.groq.com/openai/v1/chat/completions',
                headers={'Authorization': 'Bearer ' + cfg_ia['groq_key'], 'Content-Type': 'application/json'},
                json={'model': 'llama-3.1-8b-instant', 'messages': [{'role':'user','content':prompt}],
    ...
    # Mensagem personalizada fixa (configurada na tela de serviços)
    if texto_fixo:
        return texto_fixo.replace('{nome}', primeiro_nome).replace('{servico}', svc['nome']).replace('{salao}', nome_salao)

    # Fallback: banco rotativo de frases variadas (humanizado mesmo sem IA)
    frase = _random.choice(_frases_base)
    return frase.replace('{nome}', primeiro_nome).replace('{svc}', svc['nome']).replace('{salao}', nome_salao)

@app.route('/api/lembrete-semanal/status', methods=['GET'])
def lembrete_semanal_status():
    """Quem já recebeu esta semana e quem ainda está na fila."""
    sid, err = require_salon()
    if err: return err
    import datetime as _dt
    hoje = today_br()
    ini_sem = (hoje - _dt.timedelta(days=hoje.weekday())).isoformat()
    fim_sem = (hoje + _dt.timedelta(days=6 - hoje.weekday())).isoformat()
    # Quem já recebeu esta semana
    enviados = db_exec("""
        SELECT c.nome, c.tel, l.enviado_em
        FROM lembrete_semanal_log l JOIN clientes c ON c.id=l.cli_id
        WHERE l.salon_id=%s AND l.enviado_em >= %s
        ORDER BY l.enviado_em DESC
    """, (sid, ini_sem + ' 00:00:00'), 'all')
    return jsonify({'enviados': [dict(r) for r in (enviados or [])]})


@app.route('/api/lembrete-semanal/disparar', methods=['GET','POST'])
def lembrete_semanal_disparar():
    import datetime as _dt
    key = request.args.get('key', '')
    keyrow = db_exec("SELECT valor FROM sistema_global WHERE chave='cron_key'", fetch='one')
    cron_key = keyrow['valor'] if keyrow else 'musa_cron_2024'
    ...
    if key and key == cron_key:
        sids = [r['salon_id'] for r in (db_exec(
            "SELECT DISTINCT salon_id FROM servicos WHERE lembrete_semanal=1 AND ativo=1", fetch='all') or [])]
    else:
        sid, err = require_salon()
        if err: return err
        sids = [sid]
    ...
    if not forcar and dia_semana not in (2, 3):
        return jsonify({'ok': True, 'msg': 'Não é terça nem quarta, nada enviado.'})

    if key and key == cron_key:
        # Cron: responde imediatamente e envia em background (evita timeout)
        import threading as _th
        _th.Thread(target=_run_lembrete_semanal, args=(sids, hoje, dia_semana, False), daemon=True).start()
        return jsonify({'ok': True, 'msg': 'Lembrete semanal iniciado em background'})

    # Manual (botão): também roda em background para não bloquear o Gunicorn
    # (o worker tem timeout de 30s — os sleeps de 10-30s entre mensagens estouram esse limite)
    msg_override = (request.json or {}).get('msg_fixa_override', '') if request.method == 'POST' else ''
    import threading as _th
    _th.Thread(target=_run_lembrete_semanal, args=(sids, hoje, dia_semana, True, msg_override), daemon=True).start()
    return jsonify({'ok': True, 'msg': 'Envio iniciado! As mensagens serão enviadas com pausa entre elas. Atualize a tela em alguns minutos para ver o progresso.'})


def _run_lembrete_semanal(sids, hoje, dia_semana, forcar, msg_override=''):
    """Lógica de envio com pausa humana entre mensagens (10–30s).
    Usa conexão direta ao banco (não o pool do Flask) para sobreviver aos sleeps longos."""
    import datetime as _dt, time as _time, random as _random
    import psycopg2, psycopg2.extras

    def _db(sql, params=(), fetch='none'):
        """Conexão direta — reconecta se necessário, sobrevive a sleeps longos."""
    ...
    for s in sids:
        svcs_lem = _db(
            "SELECT id, nome, lembrete_semanal_modo, lembrete_semanal_texto FROM servicos WHERE salon_id=%s AND lembrete_semanal=1 AND ativo=1",
            (s,), 'all')
        if not svcs_lem: continue
        ini_sem = (hoje - _dt.timedelta(days=hoje.weekday())).isoformat()
        fim_sem = (hoje + _dt.timedelta(days=6 - hoje.weekday())).isoformat()
        salao = _db("SELECT nome FROM saloes WHERE id=%s", (s,), 'one')
        nome_salao = salao['nome'] if salao else 'nosso salão'
        cfg_ia = _db("SELECT groq_key FROM wpp_ia_config WHERE salon_id=%s", (s,), 'one')

        ja_notificado_nesta_rodada = set()
        for svc in (svcs_lem or []):
            svc = dict(svc)
            clientes = _db("""
                SELECT DISTINCT c.id, c.nome, c.tel
                FROM clientes c
                JOIN agendamentos a ON a.cli_id=c.id AND a.salon_id=c.salon_id
                WHERE c.salon_id=%s AND c.ativo=1 AND c.tel!=''
                  AND a.svc_id = %s AND a.status='concluido'
                  AND a.data >= %s
                  AND NOT EXISTS (SELECT 1 FROM lembrete_semanal_bloqueio b
                                  WHERE b.salon_id=%s AND b.cli_id=c.id)
                  AND NOT EXISTS (SELECT 1 FROM agendamentos ag2
                                  WHERE ag2.salon_id=%s AND ag2.cli_id=c.id
                                    AND ag2.data BETWEEN %s AND %s
                                    AND ag2.status NOT IN ('cancelado'))
                  AND NOT EXISTS (SELECT 1 FROM lembrete_semanal_log l
                                  WHERE l.salon_id=%s AND l.cli_id=c.id
                                    AND l.enviado_em >= NOW() - INTERVAL '14 days')
            """, (s, svc['id'], (hoje - _dt.timedelta(days=60)).isoformat(),
                  s, s, ini_sem, fim_sem, s), 'all')
            clientes = [dict(c) for c in (clientes or []) if c['id'] not in ja_notificado_nesta_rodada]
            if not forcar:
                clientes = [c for c in clientes if
                            (dia_semana == 2 and c['id'] % 2 == 0) or
                            (dia_semana == 3 and c['id'] % 2 != 0)]
            for cli in clientes:
                primeiro = cli['nome'].split(' ')[0]
                if msg_override:
                    msg = msg_override.replace('{Nome}', primeiro).replace('{nome}', primeiro)
                else:
                    msg = _gerar_msg_lembrete_semanal(s, svc, primeiro, nome_salao, cfg_ia)
                if _enviar_wpp(s, cli['tel'], msg):
                    # Cada INSERT usa conexão nova — não quebra após o sleep
                    _db("INSERT INTO lembrete_semanal_log (salon_id, cli_id) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                        (s, cli['id']))
                    total += 1
                    ja_notificado_nesta_rodada.add(cli['id'])
                # Pausa humana entre 10 e 30 segundos antes da próxima mensagem
                _time.sleep(_random.uniform(10, 30))
    return total

# ─── RETORNO ALERTAS — Lista prévia completa ─────────────────────────────────

@app.route('/api/retorno-alertas/lista-previa', methods=['GET'])
def retorno_alertas_previa():
    sid, err = require_salon()
    if err: return err
    hoje = today_br().isoformat()
    rows = db_exec("""
        SELECT ra.*, c.nome as cli_nome, c.tel as cli_tel,
            (SELECT MAX(enviado_em) FROM contato_auto_log cal
             WHERE cal.salon_id=ra.salon_id AND cal.tipo='retorno'
               AND cal.cli_id=ra.cli_id) as ultimo_envio_retorno
        FROM retorno_alertas ra
        JOIN clientes c ON c.id=ra.cli_id
        WHERE ra.salon_id=%s AND ra.realizado=0
        ORDER BY ra.data_retorno ASC
    """, (sid,), 'all')
    out = []
    for r in (rows or []):
        r = dict(r)
        dr = r.get('data_retorno', '') or ''
        if dr < hoje:
            r['status_previa'] = 'vencido'
        elif dr == hoje:
            r['status_previa'] = 'hoje'
        elif dr <= (today_br() + __import__('datetime').timedelta(days=7)).isoformat():
            r['status_previa'] = 'esta_semana'
        elif dr <= (today_br() + __import__('datetime').timedelta(days=30)).isoformat():
            r['status_previa'] = 'este_mes'
        else:
            r['status_previa'] = 'futuro'
        out.append(r)
    return jsonify(out)

# ─── RECALCULAR VISITAS ───────────────────────────────────────────────────────

@app.route('/api/clientes/recalcular-visitas', methods=['POST'])
def clientes_recalcular_visitas():
    sid, err = require_salon()
    if err: return err
    db_exec("""
        UPDATE clientes c SET ultima_visita = sub.ult
        FROM (
            SELECT cli_id, MAX(data) as ult
            FROM agendamentos
            WHERE salon_id=%s AND status='concluido' AND data IS NOT NULL AND data != ''
            GROUP BY cli_id
        ) sub
        WHERE c.id = sub.cli_id AND c.salon_id=%s
    """, (sid, sid))
    hoje = today_br().isoformat()
    db_exec("""
        UPDATE clientes SET criado_em = %s
        WHERE salon_id=%s AND (criado_em IS NULL OR criado_em = '')
          AND (ultima_visita IS NULL OR ultima_visita = '')
    """, (hoje, sid))
    db_commit()
    com_hist = db_exec("SELECT COUNT(*) as n FROM clientes WHERE salon_id=%s AND ultima_visita IS NOT NULL AND ultima_visita != ''", (sid,), 'one')
    sem_hist = db_exec("SELECT COUNT(*) as n FROM clientes WHERE salon_id=%s AND (ultima_visita IS NULL OR ultima_visita='')", (sid,), 'one')
    return jsonify({'ok': True,
        'com_historico': com_hist['n'] if com_hist else 0,
        'sem_historico': sem_hist['n'] if sem_hist else 0})

# ─── COMISSÕES — Diagnóstico ──────────────────────────────────────────────────

@app.route('/api/comissoes/diagnostico', methods=['GET'])
def comissoes_diag():
    sid, err = require_salon()
    if err: return err
    rows = db_exec("""SELECT c.id, c.valor, c.com_pct, c.ag_id,
        a.preco as ag_preco, a.svc_id, a.data,
        s.preco as svc_preco_cad, s.nome as svc_nome
        FROM comissoes c
        LEFT JOIN agendamentos a ON a.id=c.ag_id
        LEFT JOIN servicos s ON s.id=a.svc_id
        WHERE c.salon_id=%s ORDER BY c.id DESC LIMIT 20""", (sid,), 'all')
    return jsonify([dict(r) for r in (rows or [])])
...
def agendamentos():
    ...
    try:
        ...
        novo_fim = d['h_fim']
        pro = d['pro_id']
        ini_dt = novo_data + ' ' + novo_ini
        fim_dt = novo_data + ' ' + novo_fim
        bloq = db_exec("""SELECT id, motivo FROM indisponibilidades
            WHERE salon_id=%s AND (pro_id=%s OR pro_id=0 OR pro_id IS NULL)
              AND data_inicio < %s AND data_fim > %s
            LIMIT 1""", (sid, pro, fim_dt, ini_dt), 'one')
        if bloq:
...
def finalizar(aid):
    ...
    db_exec("UPDATE agendamentos SET status='concluido',pag=%s,gorjeta=%s,preco=%s,concluido_em=NOW() WHERE id=%s AND salon_id=%s", (pag,gorjeta,preco,aid,sid))
...
def despesas():
    ...
    if request.method == 'GET':
        ...
        rows = db_exec("""SELECT d.*, p.nome as pro_nome FROM despesas d
            LEFT JOIN profissionais p ON p.id=d.pro_id
            WHERE d.salon_id=%s AND d.data>=%s AND d.data<=%s ORDER BY d.data DESC""", (sid,ini,fim), 'all')
        return jsonify([dict(r) for r in rows])
    d = request.json
    eh_adto = 1 if d.get('eh_adiantamento') else 0
    status_rec = 'pendente' if eh_adto else 'reconhecido'
    db_exec("""INSERT INTO despesas (salon_id,data,descricao,valor,categoria,pago,vencimento,pro_id,eh_adiantamento,status_reconhecimento)
               VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
            (sid,d.get('data',today_br().isoformat()),d['descricao'],d.get('valor',0),d.get('categoria',''),
             d.get('pago',0),d.get('vencimento',''),d.get('pro_id',0),eh_adto,status_rec))
    db_commit()
    return jsonify({'ok': True})
...
def despesa(did):
    ...
    d = request.json
    eh_adto = 1 if d.get('eh_adiantamento') else 0
    db_exec("""UPDATE despesas SET data=%s,descricao=%s,valor=%s,categoria=%s,pago=%s,vencimento=%s,
               data_pagamento=%s,pro_id=%s,eh_adiantamento=%s WHERE id=%s AND salon_id=%s""",
            (d.get('data'),d.get('descricao'),d.get('valor',0),d.get('categoria',''),d.get('pago',0),
             d.get('vencimento',''),d.get('data_pagamento',''),d.get('pro_id',0),eh_adto,did,sid))
    db_commit()
    return jsonify({'ok': True})
...
def comissao_pagar(cid):
    ...
    if err: return err
    hoje = today_br().isoformat()
    com = db_exec("""SELECT c.*, p.nome as pro_nome, a.data FROM comissoes c
        LEFT JOIN profissionais p ON p.id=c.pro_id
        LEFT JOIN agendamentos a ON a.id=c.ag_id
        WHERE c.id=%s AND c.salon_id=%s AND c.pago=0""", (cid, sid), 'one')
    if not com:
        return jsonify({'ok': True})  # já pago ou não encontrado
    db_exec("UPDATE comissoes SET pago=1,data_pagamento=%s WHERE id=%s AND salon_id=%s",
            (hoje, cid, sid))
    # Lançar despesa individual
    pro_nome = com['pro_nome'] or 'Profissional'
    valor = round(float(com['valor'] or 0), 2)
    descricao = 'Comissão — ' + pro_nome
    db_exec("""INSERT INTO despesas (salon_id,data,descricao,valor,categoria,pago,data_pagamento,pro_id,eh_adiantamento,status_reconhecimento)
               VALUES (%s,%s,%s,%s,'Comissão',1,%s,%s,0,'reconhecido')""",
            (sid, hoje, descricao, valor, hoje, com['pro_id']))
    db_commit()
    return jsonify({'ok': True})
...
def faturamento():
    ...
    ags_row   = db_exec("SELECT COALESCE(SUM(preco),0) as t, COUNT(*) as n FROM agendamentos WHERE salon_id=%s AND data>=%s AND data<=%s AND status='concluido'", (sid,ini,fim), 'one')
    total_ags = round(float(ags_row['t'] or 0), 2)
    qtd_ags   = int(ags_row['n'] or 0)
    ticket    = round(total_ags / qtd_ags, 2) if qtd_ags > 0 else 0
    ...
    return jsonify({
        'receitas': round(receitas,2), 'despesas': round(despesas_,2),
        'lucro': round(receitas-despesas_,2),
        'total': total_ags, 'ticket_medio': ticket, 'qtd_atendimentos': qtd_ags,
        'por_pagamento': [dict(r) for r in por_pag],
        'por_profissional': [dict(r) for r in por_pro],
        'por_servico': [dict(r) for r in por_svc]
    })
...
def indisponibilidades():
    ...
    if request.method == 'GET':
        ...
        for r in rows:
            d = dict(r)
            di = (d.get('data_inicio') or '').strip()
            df = (d.get('data_fim') or '').strip()
            def split_dt(s, hora_padrao):
                if ' ' in s:
                    p = s.split(' ', 1)
                    return p[0], (p[1][:5] if len(p[1]) >= 5 else hora_padrao)
                return s, hora_padrao
            d_data, d_hini = split_dt(di, '00:00')
            _, d_hfim = split_dt(df, '23:59')
            d['data'] = d_data
            d['h_ini'] = d_hini
            d['h_fim'] = d_hfim
            if data_f:
                df_data, _ = split_dt(df, '23:59')
                if not (d_data <= data_f <= df_data):
                    continue
            out.append(d)
        return jsonify(out)
    d = request.json
...
@app.route('/api/indisponibilidades/<int:iid>', methods=['PUT'])
def indisponibilidade_edit(iid):
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    db_exec("""UPDATE indisponibilidades SET pro_id=%s, data_inicio=%s, data_fim=%s, motivo=%s
               WHERE id=%s AND salon_id=%s""",
            (d.get('pro_id'), d.get('data_inicio'), d.get('data_fim'), d.get('motivo',''), iid, sid))
    db_commit()
    return jsonify({'ok': True})
...
def comissoes_pagar_lote():
    ...
    # Buscar comissões pendentes deste lote com dados do profissional
    placeholders = ','.join(['%s'] * len(ids))
    coms = db_exec("""SELECT c.id, c.valor, c.pro_id, p.nome as pro_nome,
        a.data as ag_data FROM comissoes c
        LEFT JOIN profissionais p ON p.id=c.pro_id
        LEFT JOIN agendamentos a ON a.id=c.ag_id
        WHERE c.id IN ({}) AND c.salon_id=%s AND c.pago=0""".format(placeholders),
        list(ids) + [sid], 'all')
    if not coms:
        return jsonify({'ok': True})

    # Agrupar por profissional para criar uma despesa por pro (não uma por comissão)
    por_pro = {}
    for c in coms:
        pid = c['pro_id']
        if pid not in por_pro:
            por_pro[pid] = {'pro_nome': c['pro_nome'], 'total_bruto': 0,
                            'ids': [], 'datas': []}
        por_pro[pid]['total_bruto'] += float(c['valor'] or 0)
        por_pro[pid]['ids'].append(c['id'])
        if c['ag_data']:
            por_pro[pid]['datas'].append(c['ag_data'])
    ...
    # Para cada profissional, calcular adiantamentos e lançar despesa
    for pid, info in por_pro.items():
        bruto = round(info['total_bruto'], 2)
        pro_nome = info['pro_nome'] or 'Profissional'

        # Período das comissões deste lote
        datas = sorted([d for d in info['datas'] if d])
        ini_period = datas[0] if datas else hoje
        fim_period = datas[-1] if datas else hoje

        # Adiantamentos reconhecidos que ainda não foram descontados neste período
        adtos = db_exec("""SELECT id, valor, descricao, data FROM despesas
            WHERE salon_id=%s AND pro_id=%s AND eh_adiantamento=1
              AND status_reconhecimento='reconhecido'
              AND data BETWEEN %s AND %s""",
            (sid, pid, ini_period, fim_period), 'all')
        adtos = [dict(a) for a in (adtos or [])]
        total_adto = round(sum(float(a['valor']) for a in adtos), 2)
        liquido = round(bruto - total_adto, 2)

        # Montar descrição discriminada
        descricao = 'Comissão — ' + pro_nome
        if total_adto > 0:
            descricao += (' | Bruto: R$%.2f | Adiant. descontados: R$%.2f | Líquido: R$%.2f' % (bruto, total_adto, liquido)).replace('.', ',')

        # Lançar despesa com o valor líquido
        db_exec("""INSERT INTO despesas (salon_id,data,descricao,valor,categoria,pago,data_pagamento,pro_id,eh_adiantamento,status_reconhecimento)
                   VALUES (%s,%s,%s,%s,'Comissão',1,%s,%s,0,'reconhecido')""",
                (sid, hoje, descricao, max(liquido, 0), hoje, pid))

    db_commit()
    return jsonify({'ok': True, 'pros_pagos': len(por_pro)})
...
def fat_12meses():
    ...
    meses = []
    total_periodo = 0
    import calendar as _cal12
    for i in range(11,-1,-1):
        mes_n = hoje.month - (i % 12)
        ano_n = hoje.year - (i // 12) - (1 if mes_n <= 0 else 0)
        if mes_n <= 0: mes_n += 12
        ini = '%04d-%02d-01' % (ano_n, mes_n)
        fim = '%04d-%02d-%02d' % (ano_n, mes_n, _cal12.monthrange(ano_n, mes_n)[1])
        row = db_exec("""SELECT COALESCE(SUM(preco),0) as t, COUNT(*) as n
            FROM agendamentos WHERE salon_id=%s AND data>=%s AND data<=%s AND status='concluido'""",
            (sid,ini,fim), 'one')
        total = round(float(row['t'] or 0), 2)
        qtd = int(row['n'] or 0)
        ticket = round(total/qtd, 2) if qtd > 0 else 0
        total_periodo += total
        import datetime as _dtlbl
        lbl = _dtlbl.date(ano_n, mes_n, 1).strftime('%b/%y')
        meses.append({'mes': mes_n, 'ano': ano_n, 'label': lbl,
                      'total': total, 'qtd': qtd, 'ticket': ticket})
    media_mensal = round(total_periodo / 12, 2) if total_periodo > 0 else 0
    return jsonify({'meses': meses, 'total_periodo': round(total_periodo,2), 'media_mensal': media_mensal})
    return jsonify({'meses': meses, 'total_periodo': round(total_periodo,2), 'media_mensal': media_mensal})
...
def fat_5anos():
    ...
    for i in range(4,-1,-1):
        ...
        ini = str(ano) + '-01-01'
        fim = str(ano) + '-12-31'
        fat = db_exec("""SELECT COALESCE(SUM(preco),0) as t, COUNT(*) as n
            FROM agendamentos WHERE salon_id=%s AND data>=%s AND data<=%s AND status='concluido'""",
            (sid, ini, fim), 'one')
        desp = db_exec("SELECT COALESCE(SUM(valor),0) as t FROM despesas WHERE salon_id=%s AND data>=%s AND data<=%s",
            (sid, ini, fim), 'one')
        total = round(float(fat['t'] or 0), 2)
        qtd = int(fat['n'] or 0)
        despesas = round(float(desp['t'] or 0), 2)
        ticket = round(total/qtd, 2) if qtd > 0 else 0
        liquido = round(total - despesas, 2)
        total_periodo += total
        anos.append({'ano': str(ano), 'total': total, 'qtd': qtd,
                     'ticket': ticket, 'despesas': despesas, 'liquido': liquido})
    media_anual = round(total_periodo / 5, 2) if total_periodo > 0 else 0
    return jsonify({'anos': anos, 'total_periodo': round(total_periodo,2), 'media_anual': media_anual})

@app.route('/api/faturamento/comparativo', methods=['GET'])
def fat_comparativo():
    sid, err = require_salon()
    if err: return err
    mes_ref = request.args.get('mes', today_br().strftime('%Y-%m'))
    try:
        ano_ref, mes_num = int(mes_ref.split('-')[0]), int(mes_ref.split('-')[1])
    except Exception:
        ano_ref, mes_num = today_br().year, today_br().month
    import calendar as _cal
    def _fat_mes(ano, mes):
        ini = '%04d-%02d-01' % (ano, mes)
        fim = '%04d-%02d-%02d' % (ano, mes, _cal.monthrange(ano, mes)[1])
        fat = db_exec("""SELECT COALESCE(SUM(preco),0) as t, COUNT(*) as n,
            COUNT(DISTINCT cli_id) as clientes FROM agendamentos
            WHERE salon_id=%s AND data>=%s AND data<=%s AND status='concluido'""", (sid,ini,fim), 'one')
        desp = db_exec("SELECT COALESCE(SUM(valor),0) as t FROM despesas WHERE salon_id=%s AND data>=%s AND data<=%s",
            (sid,ini,fim), 'one')
        total = round(float(fat['t'] or 0), 2)
        qtd = int(fat['n'] or 0)
        clientes = int(fat['clientes'] or 0)
        despesas = round(float(desp['t'] or 0), 2)
        ticket = round(total/qtd, 2) if qtd > 0 else 0
        return {'rotulo': '%04d-%02d' % (ano, mes), 'total': total, 'qtd': qtd,
                'ticket': ticket, 'clientes_unicos': clientes,
                'despesas': despesas, 'liquido': round(total-despesas, 2)}
    atual = _fat_mes(ano_ref, mes_num)
    # Calcular mês anterior sem dateutil
    if mes_num == 1:
        ano_ant, mes_ant = ano_ref - 1, 12
    else:
        ano_ant, mes_ant = ano_ref, mes_num - 1
    anterior = _fat_mes(ano_ant, mes_ant)
    def _var(a, b, campo):
        va, vb = a.get(campo,0), b.get(campo,0)
        if vb == 0: return None
        return round((va - vb) / vb * 100, 1)
    variacoes = {
        'total':    _var(atual, anterior, 'total'),
        'qtd':      _var(atual, anterior, 'qtd'),
        'ticket':   _var(atual, anterior, 'ticket'),
        'clientes': _var(atual, anterior, 'clientes_unicos'),
        'liquido':  _var(atual, anterior, 'liquido'),
    }
    return jsonify({'atual': atual, 'anterior': anterior, 'variacoes': variacoes})
...
def wpp_ia_config():
    ...
    if ex:
        db_exec("""UPDATE wpp_ia_config SET ativo=%s,groq_key=%s,saudacao=%s,
                   ...
                   personalidade=%s,atualizado_em=NOW() WHERE salon_id=%s""",
                (d.get('ativo',0), d.get('groq_key',''), d.get('saudacao',''),
                 d.get('horario_ini','08:00'), d.get('horario_fim','20:00'),
                 d.get('dias_semana','1,2,3,4,5,6'), d.get('msg_fora_horario',''),
                 d.get('personalidade',''), sid))
    else:
        db_exec("""INSERT INTO wpp_ia_config (salon_id,ativo,groq_key,saudacao,horario_ini,horario_fim,dias_semana,msg_fora_horario,personalidade)
                   VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                (sid, d.get('ativo',0), d.get('groq_key',''), d.get('saudacao',''),
                 d.get('horario_ini','08:00'), d.get('horario_fim','20:00'),
                 d.get('dias_semana','1,2,3,4,5,6'), d.get('msg_fora_horario',''),
                 d.get('personalidade','')))
    db_commit()
...
def diagnostico_geral():
    """Mostra a versão do backend, o estado real de cada envio automático,
    ...
    envios = db_exec("""SELECT tipo, COUNT(*) as n, MAX(enviado_em) as ultimo
        ...
        GROUP BY tipo ORDER BY ultimo DESC""", (sid,), 'all')
    ultimos = db_exec("""SELECT cal.tipo, cal.enviado_em, c.nome as cli_nome
        FROM contato_auto_log cal LEFT JOIN clientes c ON c.id=cal.cli_id
        WHERE cal.salon_id=%s ORDER BY cal.enviado_em DESC LIMIT 20""", (sid,), 'all')
    return jsonify({
        'backend_version': BACKEND_VERSION,
        'contato_auto': [dict(c) for c in (cfgs or [])],
        'envios_ultimos_3dias': [dict(e) for e in (envios or [])],
        'ultimos_20_envios': [dict(u) for u in (ultimos or [])]
    })

@app.route('/api/emergencia/parar-tudo', methods=['GET', 'POST'])
def emergencia_parar_tudo():
    """PARADA DE EMERGÊNCIA: desativa todo o contato automático (inativo, retorno, lembrete)
    deste salão na hora. Acessível mesmo sem POST, para abrir direto no navegador logado."""
    sid, err = require_salon()
    if err: return err
    db_exec("UPDATE contato_auto_config SET ativo=0 WHERE salon_id=%s", (sid,))
    db_commit()
    return jsonify({'ok': True, 'msg': 'Todos os envios automáticos do seu salão foram desativados agora.'})
...
def _gerar_msg_contato(sid, tipo, cli_nome, cfg_ia, cfg_auto, svc_nome=''):
    ...
    if cfg_auto.get('modo') == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            ...
            if tipo == 'retorno':
                ...
                instrucao = ('Escreva uma mensagem curta e gentil de WhatsApp lembrando a cliente ' + primeiro_nome +
                             ' do horário marcado hoje no salão ' + nome_salao + '. NÃO mencione "saudade" nem "sentimos sua falta" '
                             '— ela já é cliente ativa, isso é só um lembrete de agendamento do dia. Máximo 2 linhas, 1 emoji.')
            elif tipo == 'inativo':
                ...
                instrucao = ('Escreva uma mensagem curta e simpática de WhatsApp da recepção do salão ' + nome_salao +
                             ' para ' + primeiro_nome + '. Máximo 2 linhas, 1 emoji.')
    ...
    # Mensagem fixa (ou fallback) — cada tipo tem o seu próprio texto, nunca cai no de outro
    ...
    if not base.strip():
        if tipo == 'retorno':
            ...
            base = 'Oi {nome}! 💕 Passando para lembrar do seu horário aqui no ' + nome_salao + ' hoje. Te esperamos! ✨'
        elif tipo == 'inativo':
            base = 'Oi {nome}! 💕 Sentimos sua falta aqui no ' + nome_salao + '! Que tal agendar uma visita? Vai ser um prazer te receber de novo. ✨'
        else:
            base = 'Oi {nome}! 💕 Passando para dar um oi aqui do ' + nome_salao + '. Te esperamos! ✨'
        ...
        for cfg in (cfgs or []):
            ...
            if not forcar:
                ...
                if 'tipo_manual' in dir() and tipo_manual and tipo != tipo_manual:
                    continue
            # Buscar clientes elegíveis (cada tipo com sua própria lógica — nunca cai no de outro)
            if tipo == 'retorno':
            ...
            for cli in (clientes or []):
                ...
                if not cli.get('cli_tel'):
                    diag['sem_telefone'] += 1
                    continue
                # Janela de anti-reenvio: 45 dias para inativo (evita incomodar quem já recebeu),
                # 7 dias para os demais.
                ...
                if jaenv:
                    diag['ja_contatado'] += 1
                    continue
                # TRAVA ANTI-LOOP: nunca envia o mesmo cliente+tipo de novo dentro de 20h,
                # mesmo que o cron rode várias vezes no dia (ex: configurado de hora em hora por engano).
                loop_check = db_exec("""SELECT id FROM contato_auto_log
                    WHERE salon_id=%s AND tipo=%s AND cli_id=%s AND enviado_em > NOW() - INTERVAL '20 hours'""",
                    (s, tipo, cli['cli_id']), 'one')
                if loop_check:
                    diag['ja_contatado'] += 1
                    continue
                # Registra ANTES de enviar — garante que, mesmo se o envio demorar ou o cron
                # chamar de novo em paralelo, este cliente não recebe duplicado.
        ...
        for rotulo, mins_antes in janelas:
            ...
            for ag in (ags or []):
                ...
                ja_dia = db_exec("""SELECT ll.id FROM lembrete_log ll
                    ...
                    LIMIT 1""", (s, ag['cli_id'], ag['data'], rotulo), 'one')
                if ja_dia:
...
@app.route('/api/disparo/preview', methods=['POST'])
def disparo_preview():
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    ...
    if filtro == 'ativos_30':
        clientes = db_exec("SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1 AND ultima_visita >= %s ORDER BY ultima_visita DESC", (sid, dlimite(30)), 'all')
    elif filtro == 'ativos_60':
        clientes = db_exec("SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1 AND ultima_visita >= %s ORDER BY ultima_visita DESC", (sid, dlimite(60)), 'all')
    elif filtro == 'ativos_90':
        clientes = db_exec("SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1 AND ultima_visita >= %s ORDER BY ultima_visita DESC", (sid, dlimite(90)), 'all')
    ...
    return jsonify({'ok': True, 'total': len(clientes),
                    'clientes': [{'nome': c['nome'], 'tel': c['tel']} for c in clientes]})
...
def disparo_iniciar():
    ...
    ia_ativa = 1 if d.get('ia_ativa', True) else 0
    if len(mensagem) < 5:
    ...
    def dlimite(dias): return (hoje_iso - datetime.timedelta(days=dias)).isoformat()
    if filtro == 'ativos_30':
        clientes = db_exec("SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1 AND ultima_visita >= %s", (sid, dlimite(30)), 'all')
    elif filtro == 'ativos_60':
        clientes = db_exec("SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1 AND ultima_visita >= %s", (sid, dlimite(60)), 'all')
    elif filtro == 'ativos_90':
        clientes = db_exec("SELECT id,nome,tel FROM clientes WHERE salon_id=%s AND tel!='' AND ativo=1 AND ultima_visita >= %s", (sid, dlimite(90)), 'all')
    elif filtro == 'ativos_180':
    ...
    clientes = clientes or []
    if not clientes:
        return jsonify({'ok': False, 'erro': 'Nenhum cliente encontrado para esse filtro.'})
    camp = db_exec("""INSERT INTO disparo_campanha (salon_id,titulo,mensagem,total,limite_dia,ia_ativa,status)
                      VALUES (%s,%s,%s,%s,%s,%s,'rodando') RETURNING id""",
                   (sid, titulo, mensagem, len(clientes), limite_dia, ia_ativa), 'one')
    campanha_id = camp['id']
    for c in clientes:
        db_exec("""INSERT INTO disparo_log (salon_id,campanha_id,cli_id,numero,status)
                   VALUES (%s,%s,%s,%s,'pendente') ON CONFLICT (campanha_id,cli_id) DO NOTHING""",
                (sid, campanha_id, c['id'], c['tel']))
    db_commit()
    return jsonify({'ok': True, 'campanha_id': campanha_id, 'msg': 'Disparo iniciado para ' + str(len(clientes)) + ' clientes!'})
...
def _disparo_processar_lote(sid, campanha_id, limite_lote=8):
    ...
    if not camp or camp['status'] != 'rodando':
        return {'enviados': 0}
    pendentes = db_exec("""SELECT dl.*, c.nome as cli_nome FROM disparo_log dl
        LEFT JOIN clientes c ON c.id=dl.cli_id
        WHERE dl.campanha_id=%s AND dl.status='pendente' LIMIT %s""",
        (campanha_id, limite_lote), 'all')
    enviados = 0
    for p in (pendentes or []):
        p = dict(p)
        primeiro = (p.get('cli_nome') or '').split(' ')[0]
        msg = camp['mensagem'].replace('{nome}', primeiro)
        ok = False
        try:
            ok = _enviar_wpp(sid, p['numero'], msg)
        except Exception:
            ok = False
        novo_status = 'enviado' if ok else 'falha'
        db_exec("UPDATE disparo_log SET status=%s, enviado_em=NOW() WHERE id=%s", (novo_status, p['id']))
        if ok:
        ...
        db_commit()
    restantes = db_exec("SELECT COUNT(*) as n FROM disparo_log WHERE campanha_id=%s AND status='pendente'", (campanha_id,), 'one')
...
def disparo_processar():
    ...
    cron_key = keyrow['valor'] if keyrow else 'musa_cron_2024'
    if key and key == cron_key:
        camps = db_exec("""SELECT id, salon_id FROM disparo_campanha
            WHERE status='rodando' AND criado_em > NOW() - INTERVAL '24 hours'""", fetch='all')
        db_exec("""UPDATE disparo_campanha SET status='pausada_limite'
            WHERE status='rodando' AND criado_em <= NOW() - INTERVAL '24 hours'""")
        db_commit()
        total = 0
        for c in (camps or []):
    ...
    sid, err = require_salon()
    if err: return err
    campanha_id = request.args.get('campanha_id') or (request.json or {}).get('campanha_id')
    if not campanha_id:
        return jsonify({'ok': False, 'erro': 'campanha_id obrigatório'})
    r = _disparo_processar_lote(sid, int(campanha_id), limite_lote=10)
    return jsonify({'ok': True, 'enviados': r.get('enviados', 0)})
...
def disparo_status(campanha_id):
    ...
    log = db_exec("""SELECT dl.status, dl.numero, dl.enviado_em, c.nome as cli_nome
        FROM disparo_log dl LEFT JOIN clientes c ON c.id=dl.cli_id
        WHERE dl.campanha_id=%s ORDER BY dl.id DESC LIMIT 100""", (campanha_id,), 'all')
    return jsonify({'ok': True, 'campanha': dict(camp), 'log': [dict(l) for l in (log or [])]})
...
def disparo_cancelar(campanha_id):
    ...
    db_exec("DELETE FROM disparo_log WHERE campanha_id=%s AND salon_id=%s AND status='pendente'", (campanha_id, sid))
    db_commit()
    return jsonify({'ok': True})

@app.route('/api/disparo/cancelar-tudo', methods=['POST'])
def disparo_cancelar_tudo():
    sid, err = require_salon()
    if err: return err
    db_exec("UPDATE disparo_campanha SET status='cancelada' WHERE salon_id=%s AND status IN ('rodando','pausada_limite')", (sid,))
    db_exec("DELETE FROM disparo_log WHERE salon_id=%s AND status='pendente'", (sid,))
    db_commit()
    return jsonify({'ok': True})
...
def disparo_retomar(campanha_id):
    sid, err = require_salon()
    if err: return err
    db_exec("UPDATE disparo_campanha SET status='rodando' WHERE id=%s AND salon_id=%s", (campanha_id, sid))
    db_commit()
    return jsonify({'ok': True})

@app.route('/api/disparo/campanhas', methods=['GET'])
def disparo_campanhas():
    sid, err = require_salon()
    if err: return err
    rows = db_exec("SELECT * FROM disparo_campanha WHERE salon_id=%s ORDER BY criado_em DESC LIMIT 30", (sid,), 'all')
    return jsonify([dict(r) for r in (rows or [])])

# ─── CENTRAL DE PROGRAMADOS ───────────────────────────────────────────────────

@app.route('/api/programados', methods=['GET'])
def programados_listar():
    sid, err = require_salon()
    if err: return err
    itens = []
    cfgs = db_exec("SELECT tipo, ativo, horario, dias_semana, modo FROM contato_auto_config WHERE salon_id=%s", (sid,), 'all')
    rotulos = {'inativo':'Clientes Inativos ("sentimos sua falta")', 'retorno':'Alerta de Retorno', 'lembrete':'Lembrete de Agendamento'}
    for c in (cfgs or []):
        c = dict(c)
        itens.append({
            'grupo': 'automatico', 'tipo': c['tipo'], 'titulo': rotulos.get(c['tipo'], c['tipo']),
            'ativo': c.get('ativo', 0), 'horario': c.get('horario', ''),
            'detalhe': ('Todo dia às ' + (c.get('horario') or '08:00')) if c.get('ativo') else 'Desativado',
            'pode_excluir': False
        })
    camps = db_exec("""SELECT id, titulo, status, total, enviados, falhas, criado_em
        FROM disparo_campanha WHERE salon_id=%s
        AND status IN ('rodando','pausada_limite') ORDER BY criado_em DESC""", (sid,), 'all')
    for c in (camps or []):
        c = dict(c)
        itens.append({
            'grupo': 'campanha', 'id': c['id'], 'titulo': 'Campanha: ' + (c.get('titulo') or 'Disparo'),
            'ativo': 1, 'detalhe': c['status'] + ' · ' + str(c.get('enviados',0)) + '/' + str(c.get('total',0)) + ' enviados',
            'pode_excluir': True
        })
    servs = db_exec("SELECT id, nome FROM servicos WHERE salon_id=%s AND pos_ativo=1 AND ativo=1", (sid,), 'all')
    for s in (servs or []):
        s = dict(s)
        itens.append({
            'grupo': 'pos_atendimento', 'id': s['id'], 'titulo': 'Pós-atendimento: ' + s['nome'],
            'ativo': 1, 'detalhe': '2h após concluir este serviço', 'pode_excluir': True
        })
    svcs_lem = db_exec("SELECT id, nome FROM servicos WHERE salon_id=%s AND lembrete_semanal=1 AND ativo=1", (sid,), 'all')
    for s in (svcs_lem or []):
        s = dict(s)
        itens.append({
            'grupo': 'lembrete_semanal', 'id': s['id'], 'titulo': 'Lembrete Semanal: ' + s['nome'],
            'ativo': 1, 'detalhe': 'Terça/quarta, a cada 14 dias por cliente', 'pode_excluir': True
        })
    return jsonify({'itens': itens})

@app.route('/api/programados/excluir', methods=['POST'])
def programados_excluir():
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    grupo = d.get('grupo')
    item_id = d.get('id')
    tipo = d.get('tipo')
    if grupo == 'automatico' and tipo:
        db_exec("UPDATE contato_auto_config SET ativo=0 WHERE salon_id=%s AND tipo=%s", (sid, tipo))
    elif grupo == 'campanha' and item_id:
        db_exec("UPDATE disparo_campanha SET status='cancelada' WHERE id=%s AND salon_id=%s", (item_id, sid))
        db_exec("DELETE FROM disparo_log WHERE campanha_id=%s AND salon_id=%s AND status='pendente'", (item_id, sid))
    elif grupo == 'pos_atendimento' and item_id:
        db_exec("UPDATE servicos SET pos_ativo=0 WHERE id=%s AND salon_id=%s", (item_id, sid))
    elif grupo == 'lembrete_semanal' and item_id:
        db_exec("UPDATE servicos SET lembrete_semanal=0 WHERE id=%s AND salon_id=%s", (item_id, sid))
    else:
        return jsonify({'ok': False, 'erro': 'item inválido'})
    db_commit()
    return jsonify({'ok': True})

@app.route('/api/programados/cancelar-tudo', methods=['POST'])
def programados_cancelar_tudo():
    sid, err = require_salon()
    if err: return err
    db_exec("UPDATE contato_auto_config SET ativo=0 WHERE salon_id=%s", (sid,))
    db_exec("UPDATE disparo_campanha SET status='cancelada' WHERE salon_id=%s AND status IN ('rodando','pausada_limite')", (sid,))
    db_exec("DELETE FROM disparo_log WHERE salon_id=%s AND status='pendente'", (sid,))
    db_exec("UPDATE servicos SET pos_ativo=0 WHERE salon_id=%s", (sid,))
    db_exec("UPDATE servicos SET lembrete_semanal=0 WHERE salon_id=%s", (sid,))
    db_commit()
...
def wpp_webhook(sid):
    ...
    if fora_horario:
        msg_fora = cfg.get('msg_fora_horario') or 'Olá! No momento estamos fechados. Retornaremos em breve! 😊'
        enviar_msg(numero_cliente + '@s.whatsapp.net', msg_fora)
        db_exec("INSERT INTO wpp_conversas (salon_id,numero_cliente,nome_cliente,role,content) VALUES (%s,%s,%s,'assistant',%s)",
                (sid, conv_key, nome_cliente, msg_fora))
        db_commit()
        return jsonify({'ok': True})
