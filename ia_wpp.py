...
def montar_sistema(salao, profissionais, servicos, historico_cliente=None, personalidade='', em_andamento=False):
    ...
    # Estado da conversa: primeira mensagem vs em andamento
    if em_andamento:
        bloco_estado = """
⚠️ ESTADO DA CONVERSA: EM ANDAMENTO
Esta conversa JÁ COMEÇOU (veja as mensagens anteriores). REGRAS OBRIGATÓRIAS:
- NÃO cumprimente de novo ("olá", "seja bem-vinda", "é um prazer recebê-la"). Isso já foi feito.
- NÃO pergunte de novo algo que a cliente já respondeu. Releia o que ela disse.
- Apenas CONTINUE o atendimento de onde parou, avançando para o próximo passo.
- Exemplo: se ela já disse que quer corte amanhã de manhã, o próximo passo é VERIFICAR a agenda e oferecer horários — não perguntar de novo o período.
"""
    else:
        bloco_estado = """
ESTADO DA CONVERSA: PRIMEIRO CONTATO
Esta é a primeira mensagem. Cumprimente com calor, diga o nome do salão e pergunte como pode ajudar.
"""
...
Hoje é {dia_sem}, {hoje.strftime('%d/%m/%Y')} — agora são {hora} (horário de Brasília).
{bloco_estado}
{ref_datas}
IMPORTANTE SOBRE HORÁRIOS: quando a cliente quiser agendar para HOJE, só ofereça horários que ainda não passaram (são {hora} agora). Para outros dias, qualquer horário de funcionamento serve. Quando ela disser "amanhã", "sexta", "semana que vem", use a data correta da lista acima.
{bloco_personalidade}
...
{hist_txt}
COMO CONDUZIR:
- No primeiro contato apenas: cumprimente com calor e pergunte como pode ajudar. Em mensagens seguintes, NÃO repita saudação — continue o atendimento.
- Para agendar: descubra com leveza o serviço desejado, se tem profissional de preferência, e o melhor dia/horário — uma coisa de cada vez. Assim que tiver serviço + dia, verifique a agenda.
...
COMANDOS INTERNOS (use no final da resposta, o cliente NÃO vê):
- Quando a cliente pedir para agendar ou perguntar horários disponíveis, emita: ##VERIFICAR_AGENDA##dia=YYYY-MM-DD##pro_id=N##svc_id=N## (use o profissional e serviço se já souber; senão deixe em branco). NUNCA invente horários você mesma — sempre verifique primeiro.
- Só DEPOIS que a cliente escolher um horário específico que foi oferecido, confirme com: ##AGENDAR##cli_nome=NOME##svc_id=N##pro_id=N##data=YYYY-MM-DD##hora=HH:MM##
- Emita ##AGENDAR## UMA ÚNICA VEZ, apenas quando tiver TODOS os dados (serviço, profissional, dia e hora exata escolhida pela cliente). Nunca repita o comando se já confirmou antes.
- Para cancelar: ##CANCELAR##ag_id=N##
REGRA DE OURO: nunca prometa "vou verificar" sem emitir ##VERIFICAR_AGENDA##. Quando o agendamento já estiver feito, apenas agradeça com carinho — não emita mais nenhum comando."""
...
def responder(mensagem, salao, profissionais, servicos, historico=None, conversa=None, groq_key=None, personalidade=''):
    ...
    # Adicionar contexto da conversa anterior (até 10 mensagens)
    if tem_historico:
        for item in conversa[-10:]:
            msgs.append({'role': item['role'], 'content': item['content']})
        # Lembrete forte para a IA NÃO recomeçar do zero
        msgs.append({'role': 'system', 'content': 'IMPORTANTE: a conversa acima JÁ ESTÁ EM ANDAMENTO. NÃO se apresente de novo, NÃO repita a saudação inicial, NÃO pergunte algo que a cliente já respondeu. Apenas continue de onde parou, avançando o atendimento (ex: se ela já disse o período, ofereça os horários).'})
