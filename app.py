...
def init_db():
    ...
    cur.execute("""
        ...
        VALUES ('evolution_url', ''), ('evolution_apikey', '')
...
def seed_novo_salao(salon_id, nome_salao='Meu Salão'):
    ...
    if r['n'] == 0:
        servicos = [
            ...
            ('Corte Feminino','Cabelo',70,60),
            ('Corte Masculino','Cabelo',45,30),
            ('Corte Infantil','Cabelo',40,30),
            ('Escova','Cabelo',50,45),
            ('Escova Progressiva','Cabelo',180,180),
            ('Hidratação','Cabelo',60,45),
            ('Coloração','Coloração',150,120),
            ('Mechas / Luzes','Coloração',250,180),
            ('Retoque de Raiz','Coloração',90,90),
            ('Manicure','Unhas',35,45),
            ('Pedicure','Unhas',40,45),
            ('Manicure + Pedicure','Unhas',70,90),
            ('Esmaltação em Gel','Unhas',60,60),
            ('Alongamento de Unhas','Unhas',120,120),
            ('Design de Sobrancelha','Sobrancelha',35,30),
            ('Sobrancelha + Henna','Sobrancelha',50,45),
            ('Limpeza de Pele','Estética',120,60),
            ('Maquiagem','Estética',150,60),
            ('Depilação Buço','Depilação',20,15),
            ('Depilação Perna Inteira','Depilação',80,45),
        ]
        for nome,cat,preco,dur in servicos:
            db_exec("""INSERT INTO servicos (salon_id,nome,categoria,preco,duracao_min,ativo)
                       VALUES (%s,%s,%s,%s,%s,1)""", (salon_id,nome,cat,preco,dur))
...
def importar_salao99():
    sid, err = require_salon()
    if err: return err
    return jsonify({'ok': False, 'erro': 'Importação disponível em breve'})
...
def wpp_ia_config():
    ...
    if ex:
        db_exec("""UPDATE wpp_ia_config SET ativo=%s,groq_key=%s,saudacao=%s,
                   ...
                   atualizado_em=NOW() WHERE salon_id=%s""",
                (d.get('ativo',0), d.get('groq_key',''), d.get('saudacao',''),
                 d.get('horario_ini','08:00'), d.get('horario_fim','20:00'),
                 d.get('dias_semana','1,2,3,4,5,6'), d.get('msg_fora_horario',''), sid))
    else:
        db_exec("""INSERT INTO wpp_ia_config (salon_id,ativo,groq_key,saudacao,horario_ini,horario_fim,dias_semana,msg_fora_horario)
...
def wpp_ia_desconectar():
    ...
    if conn:
        try:
            ...
            req = _ur.Request(
                conn['evolution_url'] + '/instance/logout/' + conn['instance_name'],
                headers={'apikey': conn['instance_key']},
                method='DELETE'
            )
...
def wpp_webhook(sid):
    ...
    numero_cliente = key.get('remoteJid', '').replace('@s.whatsapp.net', '').replace('@g.us', '')
    if not numero_cliente or '@g' in key.get('remoteJid', ''):
        return jsonify({'ok': True})  # Ignorar grupos
    ...
    def enviar_msg(numero, texto_resp):
        """Envia mensagem via Evolution API (compatível v1.x e v2.x)."""
        import urllib.request as _ur, json as _js
        # Limpar número (só dígitos)
        num_limpo = numero.replace('@s.whatsapp.net', '').replace('@c.us', '')
        # Formato v2.x (number + text)
        payloads = [
            {'number': num_limpo, 'text': texto_resp},
            {'number': num_limpo, 'textMessage': {'text': texto_resp}},
        ]
        for pl in payloads:
            try:
    ...
    if fora_horario:
        ...
        db_exec("INSERT INTO wpp_conversas (salon_id,numero_cliente,nome_cliente,role,content) VALUES (%s,%s,%s,'assistant',%s)",
                (sid, numero_cliente, nome_cliente, msg_fora))
        db_commit()
        return jsonify({'ok': True})
    ...
    hist_c = db_exec("""SELECT role,content FROM wpp_conversas
        WHERE salon_id=%s AND numero_cliente=%s ORDER BY criado_em DESC LIMIT 10""",
        (sid, numero_cliente), 'all')
        ...
        for cmd in (comandos or []):
            if cmd['tipo'] == 'AGENDAR':
                ...
                try:
                    ...
                    dur = svc['duracao_min'] if svc else 60
                    h_ini_ag = p.get('hora', '10:00')
                    h_fim_ag = '%02d:%02d' % (int(h_ini_ag.split(':')[0]) + dur // 60, int(h_ini_ag.split(':')[1]) + dur % 60)
                    cli_id = cli['id'] if cli else None
                    if not cli_id and nome_cliente:
                        cur = db_exec("INSERT INTO clientes (salon_id,nome,tel,ativo,criado_em) VALUES (%s,%s,%s,1,%s) RETURNING id",
                                     (sid, p.get('cli_nome', nome_cliente), numero_cliente, today_br().isoformat()), 'one')
                        cli_id = cur['id'] if cur else None
                    if cli_id:
                        db_exec("INSERT INTO agendamentos (salon_id,cli_id,pro_id,svc_id,data,h_ini,h_fim,status) VALUES (%s,%s,%s,%s,%s,%s,%s,'confirmado')",
                               (sid, cli_id, p.get('pro_id'), p.get('svc_id'), p.get('data'), h_ini_ag, h_fim_ag))
                        db_commit()
    ...
    db_exec("INSERT INTO wpp_conversas (salon_id,numero_cliente,nome_cliente,role,content) VALUES (%s,%s,%s,'assistant',%s)",
            (sid, numero_cliente, nome_cliente, resposta))
    db_commit()
    return jsonify({'ok': True})
