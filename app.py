...
def init_db():
    ...
    try:
        cur.execute("CREATE INDEX IF NOT EXISTS idx_ficha_cli ON ficha_tecnica(salon_id, cli_id, criado_em DESC)")
    except Exception:
        pass
...
def _gerar_msg_lembrete_semanal(sid, svc, primeiro_nome, nome_salao, cfg_ia):
    ...
    _frases = [
        ...
        'Oi {nome}! Ainda dá tempo pro seu {svc} aqui no {salao} essa semana 💅 Quer?',
    ]
    if modo == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            import requests as _rq
            estilos = [
                'Comece com "Oi" e faça uma pergunta direta sobre vir fazer as unhas.',
                'Comece com o nome da cliente e use uma frase curta e animada sobre as unhas.',
                'Use "E aí" ou "Bora" e mencione que tem horário disponível para as unhas.',
                'Seja direta: está passando para lembrar das unhas e pergunta se quer marcar.',
                'Use humor leve sobre deixar as unhas pra lá e convide para agendar.',
            ]
            prompt = (
                ...
                'lembrando que está na hora de vir fazer ' + svc['nome'] + ' (serviço de unhas). '
                'A mensagem DEVE mencionar o nome do salão "' + nome_salao + '". '
                'Estilo: ' + _rnd.choice(estilos) + ' '
                'REGRAS: português do Brasil informal, máximo 2 linhas, '
                'termine com convite para agendar, 1 emoji no máximo, sem linguagem formal. '
...
def _executar_lembrete(sids, hoje, dia_semana, forcar, msg_override, com_sleep):
    ...
    for s in sids:
        ...
        for svc in (svcs_lem or []):
            ...
            clientes = [dict(c) for c in (clientes or []) if c['id'] not in ja_notificado]
            if not forcar:
            ...
            for cli in clientes:
                ...
                if _enviar_wpp(s, cli['tel'], msg):
                    db_exec("INSERT INTO lembrete_semanal_log (salon_id, cli_id) VALUES (%s,%s) ON CONFLICT DO NOTHING",
                            (s, cli['id']))
                    db_commit()
                    total += 1
                    ja_notificado.add(cli['id'])
                if com_sleep:
                    import time as _t, random as _r
                    _t.sleep(_r.uniform(10, 30))
    return total
...
# ─── FICHA TÉCNICA (observações acumulativas por cliente) ────────────────────

@app.route('/api/ficha-tecnica/<int:cli_id>', methods=['GET'])
def ficha_tecnica_get(cli_id):
    sid, err = require_salon()
    if err: return err
    rows = db_exec("""
        SELECT f.*, p.nome as pro_nome_atual
        FROM ficha_tecnica f
        LEFT JOIN profissionais p ON p.id=f.pro_id
        WHERE f.salon_id=%s AND f.cli_id=%s
        ORDER BY f.criado_em DESC
    """, (sid, cli_id), 'all')
    return jsonify([dict(r) for r in (rows or [])])

@app.route('/api/ficha-tecnica/<int:cli_id>', methods=['POST'])
def ficha_tecnica_post(cli_id):
    sid, err = require_salon()
    if err: return err
    d = request.json or {}
    obs = (d.get('obs') or '').strip()
    if not obs:
        return jsonify({'ok': False, 'erro': 'Observação não pode estar vazia'})
    pro_id = d.get('pro_id') or session.get('pro_id') or 0
    pro_nome = ''
    if pro_id:
        p = db_exec("SELECT nome FROM profissionais WHERE id=%s AND salon_id=%s", (pro_id, sid), 'one')
        pro_nome = p['nome'] if p else ''
    ag_id = d.get('ag_id') or 0
    svc_nome = d.get('svc_nome') or ''
    db_exec("""INSERT INTO ficha_tecnica (salon_id, cli_id, pro_id, pro_nome, ag_id, svc_nome, obs)
               VALUES (%s,%s,%s,%s,%s,%s,%s)""",
            (sid, cli_id, pro_id, pro_nome, ag_id, svc_nome, obs))
    db_commit()
    return jsonify({'ok': True})

@app.route('/api/ficha-tecnica/<int:cli_id>/<int:fid>', methods=['DELETE'])
def ficha_tecnica_del(cli_id, fid):
    sid, err = require_salon()
    if err: return err
    db_exec("DELETE FROM ficha_tecnica WHERE id=%s AND salon_id=%s AND cli_id=%s", (fid, sid, cli_id))
    db_commit()
    return jsonify({'ok': True})
