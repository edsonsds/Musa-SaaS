...
def servicos_importar_nail():
    ...
    for nome, categoria, dur, preco in catalogo:
        ja = db_exec("SELECT id FROM servicos WHERE salon_id=%s AND LOWER(nome)=LOWER(%s) AND LOWER(categoria)=LOWER(%s)",
                     (sid, nome, categoria), 'one')
        if ja:
            existentes += 1
            continue
        db_exec("""INSERT INTO servicos (salon_id,nome,categoria,duracao_min,preco,comissao_pct,ativo)
                   VALUES (%s,%s,%s,%s,%s,%s,1)""",
                (sid, nome, categoria, dur, preco, 40))
        criados += 1
    db_commit()
    return jsonify({'ok': True, 'criados': criados, 'ja_existiam': existentes, 'total': len(catalogo)})
...
def _gerar_msg_lembrete_semanal(sid, svc, primeiro_nome, nome_salao, cfg_ia):
    ...
    _frases = [
        ...
        '{nome}! Semana boa pra fazer {svc} no {salao}! Me manda mensagem que já marco 😊',
    ...
    if modo == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            ...
            estilos = [
                ...
                'Use "E aí" ou "Bora" e mencione que tem horário disponível.',
            ...
            prompt = (
                'Você é a recepcionista do salão "' + nome_salao + '" no Brasil. '
                'Mande uma mensagem de WhatsApp para ' + primeiro_nome + ' '
                'lembrando de vir fazer ' + svc['nome'] + ' (serviço de unhas). '
                'DEVE mencionar o salão "' + nome_salao + '". '
                'Estilo: ' + _rnd.choice(estilos) + ' '
                'REGRAS: português informal, máximo 2 linhas, convite para agendar, 1 emoji. '
...
def lembrete_semanal_status():
    ...
    hoje = today_br()
    ini_sem = (hoje - _dt.timedelta(days=hoje.weekday())).isoformat()
    enviados = db_exec("""SELECT c.nome, c.tel, l.enviado_em
        FROM lembrete_semanal_log l JOIN clientes c ON c.id=l.cli_id
        WHERE l.salon_id=%s AND l.enviado_em >= %s ORDER BY l.enviado_em DESC""",
...
def lembrete_semanal_disparar():
    ...
    # Cron: thread com sleep humanizado; manual: thread sem sleep (evita timeout Gunicorn)
    import threading as _th
    com_sleep = bool(key and key == cron_key)
    _th.Thread(target=_exec_lembrete_sem,
               args=(sids, hoje, dia_semana, forcar, msg_override, com_sleep), daemon=True).start()
    return jsonify({'ok': True, 'msg': 'Envio iniciado!'})

def _exec_lembrete_sem(sids, hoje, dia_semana, forcar, msg_override, com_sleep):
    import datetime as _dt
    total = 0
    for s in sids:
        svcs_lem = db_exec(
        ...
        for svc in (svcs_lem or []):
            ...
            clientes = db_exec("""
                ...
                WHERE c.salon_id=%s AND c.ativo=1 AND c.tel!=''
                  ...
                  AND a.data >= %s
                  AND NOT EXISTS (SELECT 1 FROM lembrete_semanal_bloqueio b WHERE b.salon_id=%s AND b.cli_id=c.id)
                  AND NOT EXISTS (SELECT 1 FROM agendamentos ag2 WHERE ag2.salon_id=%s AND ag2.cli_id=c.id
                                    AND ag2.data BETWEEN %s AND %s AND ag2.status NOT IN ('cancelado'))
                  AND NOT EXISTS (SELECT 1 FROM lembrete_semanal_log l WHERE l.salon_id=%s AND l.cli_id=c.id
                                    AND l.enviado_em >= NOW() - INTERVAL '14 days')
...
def agendamentos():
    ...
    if d.get('cli_id'):
        db_exec("UPDATE retorno_alertas SET realizado=1 WHERE salon_id=%s AND cli_id=%s AND realizado=0", (sid, d['cli_id']))
    db_commit()
    return jsonify({'ok': True, 'id': cur['id']})
...
@app.route('/api/ficha-tecnica/existentes', methods=['GET'])
def ficha_tecnica_existentes():
    """Retorna a lista de cli_ids que têm pelo menos uma observação registrada.
    Usado para acender o indicador verde no ícone 🔬 sem precisar de uma chamada por cliente."""
    sid, err = require_salon()
    if err: return err
    rows = db_exec("SELECT DISTINCT cli_id FROM ficha_tecnica WHERE salon_id=%s", (sid,), 'all')
    return jsonify([r['cli_id'] for r in (rows or [])])

@app.route('/api/ficha-tecnica/<int:cli_id>', methods=['GET'])
def ficha_tecnica_get(cli_id):
    sid, err = require_salon()
    if err: return err
    rows = db_exec("""SELECT * FROM ficha_tecnica WHERE salon_id=%s AND cli_id=%s
        ORDER BY criado_em DESC""", (sid, cli_id), 'all')
    return jsonify([dict(r) for r in (rows or [])])
...
def ficha_tecnica_post(cli_id):
    ...
    if not obs:
        return jsonify({'ok': False, 'erro': 'Observação vazia'})
    ...
    db_exec("""INSERT INTO ficha_tecnica (salon_id,cli_id,pro_id,pro_nome,ag_id,svc_nome,obs)
            ...
            (sid, cli_id, pro_id, pro_nome, d.get('ag_id',0), d.get('svc_nome',''), obs))
    db_commit()
    return jsonify({'ok': True})
...
def lembrete_teste_envio():
    ...
    if err: return err
    d = request.json or {}
    numero = (d.get('numero') or '').strip().replace(' ','').replace('-','').replace('(','').replace(')','')
    if not numero:
            ...
            if texto_limpo == palavra:
                ...
                if cli_c:
                    ...
                    ag_c = db_exec("""SELECT id FROM agendamentos WHERE salon_id=%s AND cli_id=%s
                                      AND data>=%s AND status NOT IN ('cancelado','concluido')
