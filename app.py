...
def seed_novo_salao(salon_id, nome_salao='Meu Salão'):
    ...
    # Taxas
    r = db_exec("SELECT COUNT(*) FROM taxas WHERE salon_id=%s", (salon_id,), 'one')
    if r[0] == 0:
    ...
    # WhatsApp templates
    r = db_exec("SELECT COUNT(*) FROM wpp_templates WHERE salon_id=%s", (salon_id,), 'one')
    if r[0] == 0:
        tpls = [
    ...
    r = db_exec("SELECT COUNT(*) FROM usuarios WHERE salon_id=%s AND login='admin'", (salon_id,), 'one')
    if r[0] == 0:
