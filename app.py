...
def api_login():
    ...
    if not salon_id:
        # Buscar salão pelo email do usuário (cadastro self-service)
        u_any = db_exec(
            "SELECT salon_id FROM usuarios WHERE login=LOWER(%s) AND ativo=1 LIMIT 1",
            (login_val,), 'one')
        if u_any:
    ...
    u = db_exec("SELECT * FROM usuarios WHERE salon_id=%s AND (login=%s OR login=LOWER(%s)) AND ativo=1",
                (salon_id, login_val, login_val), 'one')
...
def cadastro_salao():
    ...
    db_exec(
        "INSERT INTO usuarios (salon_id,nome,login,senha_hash,perfil,permissoes,ativo) VALUES (%s,%s,%s,%s,'admin','[]',1)",
        (salon_id, nome_resp or nome_salao, email, hash_senha(senha)))
...
def importar_salao99():
    ...
    # Recebe arquivos via FormData (multipart) — campos: colaborador, servico, cliente, atendimento
    def ler_arquivo(campo):
        f = request.files.get(campo)
        if not f:
            return ''
        raw = f.read()
        # Tentar UTF-8, depois latin-1
    ...
    def parse_csv(txt):
        ...
        reader = _csv.DictReader(_io.StringIO(txt), delimiter=sep)
        rows = []
        for r in reader:
    ...
    d = {
        'colaboradores': ler_arquivo('colaborador'),
        'servicos': ler_arquivo('servico'),
        'clientes': ler_arquivo('cliente'),
        'atendimentos': ler_arquivo('atendimento'),
    }

    res = {'clientes': 0, 'servicos': 0, 'colaboradores': 0, 'atendimentos': 0, 'erros': []}
    map_cli, map_pro, map_svc = {}, {}, {}

    try:
        # ── SERVIÇOS ──
        for r in parse_csv(d.get('servicos', '')):
            nome = r.get('nome', '')
            if not nome: continue
            ex = db_exec("SELECT id FROM servicos WHERE salon_id=%s AND LOWER(nome)=LOWER(%s)", (sid, nome), 'one')
            if ex:
                map_svc[nome.lower()] = ex['id']
                continue
            try:
                dur = int(float(r.get('duracao_min', 60) or 60))
            except: dur = 60
            try:
                preco = float(r.get('preco', 0) or 0)
            except: preco = 0
            try:
                com = float(r.get('comissao_pct', 40) or 40)
            except: com = 40
            ativo = 1 if str(r.get('ativo', '1')) in ('1', 'True', 'true', 'sim') else 0
            novo = db_exec("""INSERT INTO servicos (salon_id,nome,categoria,duracao_min,preco,comissao_pct,ativo)
                              VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
                           (sid, nome, r.get('categoria', ''), dur, preco, com, ativo), 'one')
            map_svc[nome.lower()] = novo['id']
            res['servicos'] += 1
        db_commit()

        # ── COLABORADORES ──
        for r in parse_csv(d.get('colaboradores', '')):
            nome = r.get('nome', '')
            if not nome: continue
            ex = db_exec("SELECT id FROM profissionais WHERE salon_id=%s AND LOWER(nome)=LOWER(%s)", (sid, nome), 'one')
            if ex:
                map_pro[nome.lower()] = ex['id']
                continue
            try:
                com = float(r.get('comissao_pct', 40) or 40)
            except: com = 40
            ativo = 1 if str(r.get('ativo', '1')) in ('1', 'True', 'true', 'sim') else 0
            novo = db_exec("""INSERT INTO profissionais (salon_id,nome,cargo,cor,comissao_pct,h_inicio,h_fim,ativo)
                              VALUES (%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
                           (sid, nome, r.get('cargo', ''), r.get('cor', '#EC4899') or '#EC4899', com,
                            r.get('h_inicio', '08:00') or '08:00', r.get('h_fim', '20:00') or '20:00', ativo), 'one')
            map_pro[nome.lower()] = novo['id']
            res['colaboradores'] += 1
        db_commit()

        # ── CLIENTES ──
        for r in parse_csv(d.get('clientes', '')):
            nome = r.get('nome', '')
            if not nome: continue
            tel = r.get('tel', '')
            ex = db_exec("SELECT id FROM clientes WHERE salon_id=%s AND LOWER(nome)=LOWER(%s)", (sid, nome), 'one')
            if ex:
                map_cli[nome.lower()] = ex['id']
                continue
            ativo = 1 if str(r.get('ativo', '1')) in ('1', 'True', 'true', 'sim') else 0
            novo = db_exec("""INSERT INTO clientes (salon_id,nome,tel,email,cpf,nasc,obs,ativo,criado_em)
                              VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
                           (sid, nome, tel, r.get('email', ''), r.get('cpf', ''),
                            r.get('nasc', ''), r.get('obs', ''), ativo, today_br().isoformat()), 'one')
            map_cli[nome.lower()] = novo['id']
            res['clientes'] += 1
        db_commit()

        # ── ATENDIMENTOS ──
        # Recarregar mapas completos (inclui os que já existiam antes)
        for row in db_exec("SELECT id,nome FROM clientes WHERE salon_id=%s", (sid,), 'all'):
            map_cli[row['nome'].lower()] = row['id']
        for row in db_exec("SELECT id,nome FROM profissionais WHERE salon_id=%s", (sid,), 'all'):
            map_pro[row['nome'].lower()] = row['id']
        for row in db_exec("SELECT id,nome FROM servicos WHERE salon_id=%s", (sid,), 'all'):
            map_svc[row['nome'].lower()] = row['id']

        ats = parse_csv(d.get('atendimentos', ''))
        # Carregar atendimentos já existentes UMA vez (evita 7900 SELECTs)
        existentes = set()
        for row in db_exec("SELECT cli_id,data,h_ini FROM agendamentos WHERE salon_id=%s", (sid,), 'all'):
            existentes.add((row['cli_id'], row['data'], row['h_ini']))

        cont = 0
        for r in ats:
            try:
                cli_nome = (r.get('cliente', '') or '').lower()
                pro_nome = (r.get('colaborador', '') or '').lower()
                svc_nome = (r.get('servico', '') or '').lower()
                data = r.get('data', '')
                h_ini = r.get('horario_inicio', '') or r.get('horario', '')
                h_fim = r.get('horario_termino', '') or h_ini
                if not (cli_nome and data and h_ini):
                    continue
                cli_id = map_cli.get(cli_nome)
                pro_id = map_pro.get(pro_nome, 0)
                svc_id = map_svc.get(svc_nome, 0)
                if not cli_id:
                    novo = db_exec("INSERT INTO clientes (salon_id,nome,ativo,criado_em) VALUES (%s,%s,1,%s) RETURNING id",
                                   (sid, r.get('cliente', 'Cliente'), today_br().isoformat()), 'one')
                    cli_id = novo['id']; map_cli[cli_nome] = cli_id
                ...
                try:
                    valor = float(r.get('valor_total', 0) or r.get('valor_subtotal', 0) or 0)
                except: valor = 0
                status = r.get('status', 'agendado') or 'agendado'
                db_exec("""INSERT INTO agendamentos (salon_id,cli_id,pro_id,svc_id,data,h_ini,h_fim,preco,status,pag,obs)
                           VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                        (sid, cli_id, pro_id, svc_id, data, h_ini, h_fim, valor, status,
                         r.get('pagamento', ''), r.get('obs', '')))
                res['atendimentos'] += 1
                cont += 1
                # Commit em lotes de 500 (evita transação gigante)
                if cont % 500 == 0:
                    db_commit()
            except Exception as ex_linha:
                db_rollback()
                res['erros'].append(str(ex_linha)[:100])
                continue
        db_commit()
        db_commit()
        ...
        db_rollback()
        import traceback
        return jsonify({'ok': False, 'erro': str(ex), 'traceback': traceback.format_exc()[-500:], 'parcial': res})
...
def auth_recuperar():
    return jsonify({'ok': False, 'erro': 'Recuperação de senha em breve'})

@app.route('/api/auth/resetar', methods=['POST'])
def auth_resetar():
    return jsonify({'ok': False, 'erro': 'Reset de senha em breve'})
...
def _gerar_msg_contato(sid, tipo, cli_nome, cfg_ia, cfg_auto):
    ...
    if cfg_auto.get('modo') == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            ...
            if tipo == 'retorno':
                instrucao = 'Escreva uma mensagem curta e calorosa de WhatsApp para lembrar a cliente ' + primeiro_nome + ' de que está na hora de voltar ao salão ' + nome_salao + ' para o retorno do tratamento. Seja gentil, no máximo 3 linhas, com 1 emoji.'
            else:
                instrucao = 'Escreva uma mensagem curta e carinhosa de WhatsApp sentindo saudade da cliente ' + primeiro_nome + ', que não vem ao salão ' + nome_salao + ' há um tempo. Convide-a a voltar, sem cobrança. Máximo 3 linhas, 1 emoji.'
    ...
    return base.replace('{nome}', primeiro_nome)
    ...
    for s in sids:
        ...
        for cfg in (cfgs or []):
            ...
            if tipo == 'retorno':
                ...
                clientes = db_exec("""SELECT ra.id as alerta_id, ra.cli_id, c.nome as cli_nome, c.tel as cli_tel
            ...
            for cli in (clientes or []):
                ...
                if not cli.get('cli_tel'):
                    diag['sem_telefone'] += 1
                    continue
                # Evitar reenvio: já contatado nos últimos 7 dias para esse tipo?
                jaenv = db_exec("""SELECT id FROM contato_auto_log WHERE salon_id=%s AND tipo=%s AND cli_id=%s
                                   AND enviado_em > NOW() - INTERVAL '7 days'""", (s, tipo, cli['cli_id']), 'one')
                if jaenv:
                    diag['ja_contatado'] += 1
                    continue
                msg = _gerar_msg_contato(s, tipo, cli.get('cli_nome',''), dict(cfg_ia) if cfg_ia else {}, cfg)
...
def wpp_webhook(sid):
    ...
    cli = db_exec("SELECT id FROM clientes WHERE salon_id=%s AND tel LIKE %s", (sid, '%' + numero_cliente[-8:] + '%'), 'one')
    hist_ags = []
    if cli:
    ...
    try:
        ...
        if erro or not resposta:
            # Registrar o erro real para diagnóstico
            try:
                ...
                pass
            resposta = 'Desculpe, tive um problema técnico. Por favor, ligue para nós! 🙏'
                ...
                try:
                    ...
                    if not cli_id:
                        cur = db_exec("INSERT INTO clientes (salon_id,nome,tel,ativo,criado_em) VALUES (%s,%s,%s,1,%s) RETURNING id",
                                     (sid, p.get('cli_nome', nome_cliente) or 'Cliente WhatsApp', numero_cliente, today_br().isoformat()), 'one')
