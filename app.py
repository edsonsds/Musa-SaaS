...
def _importar_salao99_inner(sid):
    ...
    try:
        ...
        if rows:
            ...
            # Descobre quais serviços/categorias cada profissional realizou e habilita na ficha dele
            try:
                # Info dos serviços: id -> (categoria, comissao_pct)
                ...
                # Quais serviços cada profissional realizou
                ...
                for pro_id, svc_ids in pro_svcs.items():
                    # Categorias únicas que o profissional atende
                    ...
                    # pro_svc_config (comissão por serviço)
                    cur.execute("DELETE FROM pro_svc_config WHERE salon_id=%s AND pro_id=%s", (sid, pro_id))
                    cfg_rows = []
                    for s in svc_ids:
                        if s not in svc_info: continue
                        cfg_rows.append((sid, pro_id, svc_info[s]['cat'], s, svc_info[s]['pct']))
                    if cfg_rows:
                        execute_values(cur,
                            "INSERT INTO pro_svc_config (salon_id,pro_id,categoria,svc_id,comissao_override) VALUES %s",
                            cfg_rows, page_size=500)
                conn.commit()
