...
def montar_sistema(salao, profissionais, servicos, historico_cliente=None, personalidade='', em_andamento=False):
    ...
    # Cliente novo ou conhecido
    if (salao or {}).get('_cliente_novo'):
        bloco_cliente = "\nCLIENTE: é a primeira vez desta pessoa (sem cadastro). Antes de confirmar o agendamento, PERGUNTE o nome completo dela (nome e sobrenome) com gentileza, e use esse nome no comando ##AGENDAR## (cli_nome=Nome Sobrenome).\n"
    elif (salao or {}).get('_cliente_nome'):
        bloco_cliente = "\nCLIENTE: já é cadastrada, o nome dela é " + str((salao or {}).get('_cliente_nome')) + ". Trate-a pelo primeiro nome com carinho e use esse nome no ##AGENDAR##.\n"
    else:
        bloco_cliente = ''
...
{bloco_estado}{bloco_cliente}{bloco_personalidade}
COMO CONVERSAR:
...
- Para registrar um agendamento confirmado, inclua no FINAL da mensagem (a cliente não vê): ##AGENDAR##cli_nome=NOME##svc_id=N##pro_id=N##data=YYYY-MM-DD##hora=HH:MM## — use os números [id=N] das listas acima. Faça isso UMA vez, só quando tiver todos os dados. Se a cliente for nova, só emita o comando DEPOIS que ela disser o nome completo.
