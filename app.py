...
def contato_auto_set():
    ...
    d = request.json or {}
    tipo = d.get('tipo', '')
    if tipo not in ('retorno', 'inativo'):
    ...
    if ex:
        db_exec("""UPDATE contato_auto_config SET ativo=%s,modo=%s,msg_fixa=%s,horario=%s,
                   dias_semana=%s,dias_inativo=%s,atualizado_em=NOW() WHERE salon_id=%s AND tipo=%s""",
                (d.get('ativo',0), d.get('modo','fixo'), d.get('msg_fixa',''), d.get('horario','08:00'),
                 d.get('dias_semana','1,2,3,4,5'), d.get('dias_inativo',40), sid, tipo))
    else:
        db_exec("""INSERT INTO contato_auto_config (salon_id,tipo,ativo,modo,msg_fixa,horario,dias_semana,dias_inativo)
                   VALUES (%s,%s,%s,%s,%s,%s,%s,%s)""",
                (sid, tipo, d.get('ativo',0), d.get('modo','fixo'), d.get('msg_fixa',''),
                 d.get('horario','08:00'), d.get('dias_semana','1,2,3,4,5'), d.get('dias_inativo',40)))
    db_commit()
    return jsonify({'ok': True})
...
def contato_auto_disparar():
    ...
    for s in sids:
        ...
        for cfg in (cfgs or []):
            ...
            db_commit()
            detalhes.append({'salon': s, 'tipo': tipo, 'enviados': enviados_tipo})
