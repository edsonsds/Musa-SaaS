...
def _gerar_msg_lembrete_semanal(sid, svc, primeiro_nome, nome_salao, cfg_ia):
    ...
    if modo == 'ia' and cfg_ia and cfg_ia.get('groq_key'):
        try:
            ...
            prompt = (
                ...
                'REGRAS OBRIGATÓRIAS: '
                '1. Escreva em português do Brasil informal, como uma amiga escreveria — use "tá", "né", "a gente" se soar natural. '
                '2. Máximo 2 linhas. '
                '3. Termine com um convite claro para remarcar (ex: "Quer marcar?" ou "Vou reservar seu horário?"). '
                '4. Use 1 emoji no máximo. '
                '5. NÃO use palavras como "prezada", "cordialmente", "atenciosamente" nem linguagem formal. '
                '6. Não mencione "saudade", não seja dramática. Seja direta e simpática. '
            ...
            r = _rq.post('https://api.groq.com/openai/v1/chat/completions',
                ...
                json={'model': 'llama-3.1-8b-instant', 'messages': [{'role':'user','content':prompt}],
                      'max_tokens': 120, 'temperature': 0.75}, timeout=15)
            if r.status_code == 200:
    ...
    return ('Oi ' + primeiro_nome + '! 💅 Vai fazer ' + svc['nome'] + ' essa semana? '
            'Me fala que já reservo seu horário!')
