...
def _importar_salao99_inner(sid):
    ...
    try:
        # 1. COLABORADORES
        for row in read_csv('colaborador'):
            ...
            if ex:
                ...
                try:
                    ...
                    novo = db_exec("""INSERT INTO profissionais (salon_id,nome,cargo,cor,comissao_pct,h_inicio,h_fim,ativo)
                                   ...
                                   (sid, nome, row.get('cargo','') or '', row.get('cor','#EC4899') or '#EC4899',
                                    com, row.get('h_inicio','08:00') or '08:00', row.get('h_fim','20:00') or '20:00'), 'one')
                    pro_map[nome] = novo['id']; res['colaboradores'] += 1
                except Exception as e:
                    db_rollback(); res['erros'].append('Colab ' + nome + ': ' + str(e)[:80])
        db_commit()

        # 2. SERVIÇOS
        for row in read_csv('servico'):
            ...
            if ex:
                ...
                try:
                    ...
                    novo = db_exec("""INSERT INTO servicos (salon_id,nome,categoria,duracao_min,preco,comissao_pct,ativo)
                                   ...
                                   (sid, nome, cat, dur, preco, com), 'one')
                    svc_map[nome] = novo['id']; res['servicos'] += 1
                except Exception as e:
                    db_rollback(); res['erros'].append('Svc ' + nome + ': ' + str(e)[:80])
        db_commit()

        # 3. CLIENTES
        for row in read_csv('cliente'):
            id_ext = str(row.get('id_cliente') or row.get('id') or '').strip()
            nome = (row.get('nome') or row.get('Nome') or '').strip()
            if not nome: continue
            tel = (row.get('tel') or row.get('telefone_1') or row.get('Telefone 1') or
                   row.get('celular') or row.get('telefone') or '').strip()
            ex = None
            if tel:
                ex = db_exec("SELECT id FROM clientes WHERE salon_id=%s AND tel=%s AND tel!=''", (sid, tel), 'one')
            if not ex:
            ...
            if ex:
                if id_ext: cli_map[id_ext] = ex['id']
                cli_map[nome.upper()] = ex['id']
            else:
                try:
                    email = (row.get('email') or row.get('E-mail') or '').strip()
                    nasc = (row.get('nasc') or row.get('data_nascimento') or '').strip()
                    cpf = (row.get('cpf') or row.get('CPF') or '').strip()
                    novo = db_exec("""INSERT INTO clientes (salon_id,nome,tel,email,cpf,nasc,ativo,criado_em)
                                      VALUES (%s,%s,%s,%s,%s,%s,1,%s) RETURNING id""",
                                   (sid, nome, tel, email, cpf, nasc, today_br().isoformat()), 'one')
                    lid = novo['id']
                    if id_ext: cli_map[id_ext] = lid
                    cli_map[nome.upper()] = lid
                    res['clientes'] += 1
                except Exception as e:
                    db_rollback(); res['erros'].append('Cli ' + nome + ': ' + str(e)[:80])
        db_commit()

        # 4. ATENDIMENTOS
        ats = read_csv('atendimento')
        if ats:
            # Recarregar mapas completos do banco
            for r2 in db_exec("SELECT id,nome FROM profissionais WHERE salon_id=%s", (sid,), 'all'):
                pro_map[r2['nome']] = r2['id']
            for r2 in db_exec("SELECT id,nome FROM servicos WHERE salon_id=%s", (sid,), 'all'):
                svc_map[r2['nome']] = r2['id']
            for r2 in db_exec("SELECT id,nome FROM clientes WHERE salon_id=%s", (sid,), 'all'):
                cli_map[r2['nome'].upper()] = r2['id']
            fb_pro = db_exec("SELECT id FROM profissionais WHERE salon_id=%s ORDER BY id LIMIT 1", (sid,), 'one')
            fb_pro_id = fb_pro['id'] if fb_pro else 0
            fb_svc = db_exec("SELECT id FROM servicos WHERE salon_id=%s ORDER BY id LIMIT 1", (sid,), 'one')
            fb_svc_id = fb_svc['id'] if fb_svc else 0

            # Anti-duplicação em memória
            existentes = set()
            for r2 in db_exec("SELECT cli_id,data,h_ini FROM agendamentos WHERE salon_id=%s", (sid,), 'all'):
                existentes.add((r2['cli_id'], r2['data'], r2['h_ini']))

            # Coletar todos os registros e inserir em massa (rápido, evita timeout)
            registros = []
            for i, row in enumerate(ats):
                try:
                    id_ext = str(row.get('id_cliente', '') or '').strip()
                    cli_nome = (row.get('cliente') or '').strip().upper()
                    data = parse_data((row.get('data', '') or '').strip())
                    if not data or len(data) != 10:
                        continue
                    col_nome = (row.get('colaborador') or '').strip()
                    svc_nome = (row.get('servico') or '').strip()
                    ...
                    cli_id = cli_map.get(id_ext) or cli_map.get(cli_nome) or 0
                    if not cli_id:
                        continue
                    ...
                    if not pro_id:
                        ...
                        for k, v in pro_map.items():
                            if primeiro and primeiro.lower() in k.lower():
                                pro_id = v; break
                        if not pro_id:
                            pro_id = fb_pro_id
                    ...
                    if not svc_id:
                        for k, v in svc_map.items():
                            if svc_nome and svc_nome.lower() in k.lower():
                                svc_id = v; break
                        if not svc_id:
                            svc_id = fb_svc_id
                    ...
                    if len(res['erros']) < 20:
                        res['erros'].append('Linha ' + str(i + 2) + ': ' + str(e)[:80])

            # Inserção em massa com execute_values (centenas de vezes mais rápido)
            if registros:
                try:
                    ...
                    db_rollback()
                    # Fallback: inserir um a um se o bulk falhar
                    cont = 0
                    for reg in registros:
                        try:
                            db_exec("""INSERT INTO agendamentos (salon_id,cli_id,pro_id,svc_id,data,h_ini,h_fim,preco,status)
                                       VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)""", reg)
                            cont += 1
                            if cont % 500 == 0: db_commit()
                        except Exception:
                            db_rollback()
                    db_commit()
                    res['atendimentos'] = cont
